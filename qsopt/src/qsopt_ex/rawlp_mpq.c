...
ILL_PTRWORLD_LISTFREE_ROUTINE (mpq_colptr, colptr_listfree, colptrfree)
ILL_PTRWORLD_LEAKS_ROUTINE (mpq_colptr, colptr_check_leaks, this_val, int)
...
int mpq_ILLraw_add_ranges_coef (
	mpq_rawlpdata * lp,
	int rowind,
	mpq_t coef)
{
...
int mpq_ILLraw_is_mem_other_sos (
	mpq_rawlpdata * lp,
	int colind)
{
...
const char *mpq_ILLraw_set_lowerBound (
	mpq_rawlpdata * lp,
	int i,
	mpq_t bnd)
{
...
const char *mpq_ILLraw_set_upperBound (
	mpq_rawlpdata * lp,
	int i,
	mpq_t bnd)
{
	...
	lp->ubind[i] = (char) 1;
	if (!mpq_EGlpNumIsNeqqZero (lp->lower[i]) &&
			!mpq_EGlpNumIsNeqqZero (bnd))
	{
...
const char *mpq_ILLraw_set_fixedBound (
	mpq_rawlpdata * lp,
	int i,
	mpq_t bnd)
{
...
const char *mpq_ILLraw_set_unbound (
	mpq_rawlpdata * lp,
	int i)
{
	ILL_FAILtrue_no_rval (i >= lp->ncols, "proper colind");
...
const char *mpq_ILLraw_set_binaryBound (
	mpq_rawlpdata * lp,
	int i)
{
	ILL_FAILtrue_no_rval (i >= lp->ncols, "proper colind");
	...
	mpq_rawlpdata * lp)
{
	...
	for (i = 0; i < lp->ncols; i++)
	{
		if (!lp->lbind[i])
		{
			if (lp->ubind[i] && mpq_EGlpNumIsLessZero (lp->upper[i]))
			{
				mpq_EGlpNumCopy (lp->lower[i], mpq_ILL_MINDOUBLE);
			}
		}
...
int mpq_ILLraw_first_nondefault_bound (
	mpq_ILLlpdata * lp)
{
	int ri = lp->nstruct, i;
	...
	{
		i = lp->structmap[ri];
		if (!mpq_ILLraw_default_lower (lp, i) || !mpq_ILLraw_default_upper (lp, i, ri))
			break;
	}
CLEANUP:
	return ri;
	...
	int i)
{
	...
	if (!mpq_EGlpNumIsNeqqZero (lp->lower[i]) &&
			!mpq_EGlpNumIsLessZero (lp->upper[i]))
	{
		return 1;
	}
...
int mpq_ILLraw_fill_in_rownames (
	mpq_rawlpdata * lp)
{
	int i, rval = 0;
	...
	int *colindex)
{
	...
	for (i = 0; i < raw->ncols; i++)
	{
		...
		if (ci != -1)
		{
			...
			ILL_UTIL_STR (lp->colnames[ci], mpq_ILLraw_colname (raw, i));
	...
	int *rowindex)
{
	...
	if (lp->nrows > 0)
	{
		...
		safeRegister (&lp->rowtab, mpq_ILLraw_rowname (raw, raw->objindex), 0);
		...
		for (i = 0; i < raw->nrows; i++)
		{
			...
			if (i == raw->refrowind)
			{
				ILL_UTIL_STR (lp->refrowname, mpq_ILLraw_rowname (raw, i));
				lp->refind = ri;
			}
			if (raw->rowsense[i] != 'N')
			{
				...
				ILL_UTIL_STR (lp->rownames[ri], mpq_ILLraw_rowname (raw, i));
			...
			else if (i == objind)
			{
				...
				ILL_UTIL_STR (lp->objname, mpq_ILLraw_rowname (raw, i));
			}
			else
			{
				/* unused 'N' row */
	...
	int *rowindex)
{
	...
	for (cp = raw->ranges; cp; cp = cp->next)
	{
		...
		case 'E':
			lp->sense[ri] = 'R';
			if (!mpq_EGlpNumIsLessZero (cp->coef))
			{
	...
	int *colindex)
{
	...
	/* build sos info */
	/* see comment in lpdata_mpq.h about mpq_ILLlpdata's sos and is_sos_mem 
...
int mpq_ILLrawlpdata_to_lpdata (
	mpq_rawlpdata * raw,
	mpq_ILLlpdata * lp)
{
	int rval = 0;
...
int mpq_ILLraw_set_bounds_name (
	mpq_rawlpdata * lp,
	const char *name,
	int *skip)
{
...
int mpq_ILLraw_set_ranges_name (
	mpq_rawlpdata * lp,
	const char *name,
	int *skip)
{
	...
	if (lp)
	{
		...
		if (lp->ncols > 0)
		{
			...
			for (i = 0; i < lp->ncols; i++)
			{
				...
				{
					printf ("%s: ", mpq_ILLraw_rowname (lp, cp->this_val));
					printf ("%c ", (mpq_EGlpNumIsLessZero (cp->coef)) ? '-' : '+');
		...
		if (lp->rangesname)
		{
			...
			{
				printf ("(%s, %f) ", mpq_ILLraw_rowname (lp, cp->this_val),
