...
ILL_PTRWORLD_LISTFREE_ROUTINE (dbl_colptr, colptr_listfree, colptrfree)
ILL_PTRWORLD_LEAKS_ROUTINE (dbl_colptr, colptr_check_leaks, this_val, int)
...
int dbl_ILLraw_add_ranges_coef (
	dbl_rawlpdata * lp,
	int rowind,
	double coef)
{
...
int dbl_ILLraw_is_mem_other_sos (
	dbl_rawlpdata * lp,
	int colind)
{
...
const char *dbl_ILLraw_set_lowerBound (
	dbl_rawlpdata * lp,
	int i,
	double bnd)
{
...
const char *dbl_ILLraw_set_upperBound (
	dbl_rawlpdata * lp,
	int i,
	double bnd)
{
	...
	lp->ubind[i] = (char) 1;
	if (!dbl_EGlpNumIsNeqqZero (lp->lower[i]) &&
			!dbl_EGlpNumIsNeqqZero (bnd))
	{
...
const char *dbl_ILLraw_set_fixedBound (
	dbl_rawlpdata * lp,
	int i,
	double bnd)
{
...
const char *dbl_ILLraw_set_unbound (
	dbl_rawlpdata * lp,
	int i)
{
	ILL_FAILtrue_no_rval (i >= lp->ncols, "proper colind");
...
const char *dbl_ILLraw_set_binaryBound (
	dbl_rawlpdata * lp,
	int i)
{
	ILL_FAILtrue_no_rval (i >= lp->ncols, "proper colind");
	...
	dbl_rawlpdata * lp)
{
	...
	for (i = 0; i < lp->ncols; i++)
	{
		if (!lp->lbind[i])
		{
			if (lp->ubind[i] && dbl_EGlpNumIsLessZero (lp->upper[i]))
			{
				dbl_EGlpNumCopy (lp->lower[i], dbl_ILL_MINDOUBLE);
			}
		}
...
int dbl_ILLraw_first_nondefault_bound (
	dbl_ILLlpdata * lp)
{
	int ri = lp->nstruct, i;
	...
	{
		i = lp->structmap[ri];
		if (!dbl_ILLraw_default_lower (lp, i) || !dbl_ILLraw_default_upper (lp, i, ri))
			break;
	}
CLEANUP:
	return ri;
	...
	int i)
{
	...
	if (!dbl_EGlpNumIsNeqqZero (lp->lower[i]) &&
			!dbl_EGlpNumIsLessZero (lp->upper[i]))
	{
		return 1;
	}
...
int dbl_ILLraw_fill_in_rownames (
	dbl_rawlpdata * lp)
{
	int i, rval = 0;
	...
	int *colindex)
{
	...
	for (i = 0; i < raw->ncols; i++)
	{
		...
		if (ci != -1)
		{
			...
			ILL_UTIL_STR (lp->colnames[ci], dbl_ILLraw_colname (raw, i));
	...
	int *rowindex)
{
	...
	if (lp->nrows > 0)
	{
		...
		safeRegister (&lp->rowtab, dbl_ILLraw_rowname (raw, raw->objindex), 0);
		...
		for (i = 0; i < raw->nrows; i++)
		{
			...
			if (i == raw->refrowind)
			{
				ILL_UTIL_STR (lp->refrowname, dbl_ILLraw_rowname (raw, i));
				lp->refind = ri;
			}
			if (raw->rowsense[i] != 'N')
			{
				...
				ILL_UTIL_STR (lp->rownames[ri], dbl_ILLraw_rowname (raw, i));
			...
			else if (i == objind)
			{
				...
				ILL_UTIL_STR (lp->objname, dbl_ILLraw_rowname (raw, i));
			}
			else
			{
				/* unused 'N' row */
	...
	int *rowindex)
{
	...
	for (cp = raw->ranges; cp; cp = cp->next)
	{
		...
		case 'E':
			lp->sense[ri] = 'R';
			if (!dbl_EGlpNumIsLessZero (cp->coef))
			{
	...
	int *colindex)
{
	...
	/* build sos info */
	/* see comment in lpdata_dbl.h about dbl_ILLlpdata's sos and is_sos_mem 
...
int dbl_ILLrawlpdata_to_lpdata (
	dbl_rawlpdata * raw,
	dbl_ILLlpdata * lp)
{
	int rval = 0;
...
int dbl_ILLraw_set_bounds_name (
	dbl_rawlpdata * lp,
	const char *name,
	int *skip)
{
...
int dbl_ILLraw_set_ranges_name (
	dbl_rawlpdata * lp,
	const char *name,
	int *skip)
{
	...
	if (lp)
	{
		...
		if (lp->ncols > 0)
		{
			...
			for (i = 0; i < lp->ncols; i++)
			{
				...
				{
					printf ("%s: ", dbl_ILLraw_rowname (lp, cp->this_val));
					printf ("%c ", (dbl_EGlpNumIsLessZero (cp->coef)) ? '-' : '+');
		...
		if (lp->rangesname)
		{
			...
			{
				printf ("(%s, %f) ", dbl_ILLraw_rowname (lp, cp->this_val),
