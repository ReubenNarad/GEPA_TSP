...
#define ILL_PRE_FEAS_TOL mpf_PFEAS_TOLER	//(1e-6)
#define ILL_PRE_ZERO_TOL mpf_PIVOT_TOLER	//(1e-10)
	...
	mpf_ILLlpdata * lp)
{
	...
	if (!lp)
	{
		QSlog("mpf_ILLlp_add_logicals called with a NULL pointer");
		rval = 1;
		goto CLEANUP;
	}

	QSlog("mpf_ILLlp_add_logicals ...");
	...
	for (i = 0; i < nrows; i++)
	{
		...
		case 'R':									/* Range     */
			...
			break;
		default:
			QSlog("unknown sense %c in mpf_ILLlp_add_logicals", sense[i]);
			rval = 1;
			goto CLEANUP;
		}
		ncols++;
	...
	mpf_ILLlpdata * lp)
{
	...
	for (j = 0; j < nstruct; j++)
	{
		...
		{
			row = A->matind[k];
			if (mpf_EGlpNumIsGreatZero (gama[row]))
			{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (mpf_EGlpNumIsGreatZero ( gama[i]))
		{
	...
	int pre_types)
{
	...
	if (!lp)
	{
		QSlog("mpf_ILLlp_presolve called with a NULL pointer");
		rval = 1;
		goto CLEANUP;
	}
	...
	mpf_ILLlp_preline * line)
{
	...
	if (line->row_or_col == 0)
	{
		...
		{
			QSlog(" C%d->%g", line->ind[k], mpf_EGlpNumToLf (line->val[k]));
		}
		QSlog(" RHS->%g", mpf_EGlpNumToLf (line->rhs));
	}
	else
	{
		for (k = 0; k < line->count; k++)
		{
			QSlog(" R%d->%g", line->ind[k], mpf_EGlpNumToLf (line->val[k]));
		}
		QSlog(" Obj->%g  LB->%g  UB->%g", mpf_EGlpNumToLf (line->obj),
	...
	for (j = 0; j < ncols; j++)
	{
		if (cols[j].del == 0)
		{
			...
			if (k == cols[j].deg)
			{
				...
				if (!mpf_EGlpNumIsNeqZero (objtmp, ILL_PRE_FEAS_TOL))
				{
					set_fixed_variable (G, j, cols[j].lower);
				}
				else if (mpf_EGlpNumIsGreatZero (objtmp))
				{
					if (mpf_EGlpNumIsEqqual (cols[j].lower, mpf_ILL_MINDOUBLE))
					{
						QSlog("unbounded prob detected in empty_columns");
						QSlog("col %d, obj %g", j, mpf_EGlpNumToLf (cols[j].obj));
						rval = 1;
						goto CLEANUP;
					}
					else
				...
				else if (mpf_EGlpNumIsLessZero (objtmp))
				{
	...
	int *hit)
{
	...
	*hit = 0;
	...
	while (list)
	{
		...
		if (tdeg[rowindex] == 0)
		{
			if (mpf_EGlpNumIsNeqZero (r->rhs, ILL_PRE_FEAS_TOL))
			{
				QSlog("infeasible row detected in singleton_row");
				QSlog("empty row with rhs = %g", mpf_EGlpNumToLf (r->rhs));
				rval = 1;
				goto CLEANUP;
			}
		...
		else
		{
			...
			if (mpf_EGlpNumIsSumLess (val, ILL_PRE_FEAS_TOL, c->lower) ||
					mpf_EGlpNumIsSumLess (c->upper, ILL_PRE_FEAS_TOL, val))
			{
	...
	*hit = 0;
	...
	for (i = 0; i < nrows; i++)
	{
		if (rows[i].del == 0)
		{
			...
			if (mpf_EGlpNumIsSumLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ||
					mpf_EGlpNumIsSumLess (ub, ILL_PRE_FEAS_TOL, rows[i].rhs))
			{
			...
			else if (mpf_EGlpNumIsDiffLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ||
							 mpf_EGlpNumIsDiffLess (ub, ILL_PRE_FEAS_TOL, rows[i].rhs))
			{
				(*hit)++;
				ts = (mpf_EGlpNumIsDiffLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ? 0 : 1);
				for (k = 0; k < rows[i].deg; k++)
				{
					e = rows[i].adj[k];
					if (e->del == 0)
					{
						...
						if ((ts == 0 && mpf_EGlpNumIsLessZero (e->coef)) ||
								(ts == 1 && mpf_EGlpNumIsGreatZero (e->coef)))
						{
							set_fixed_variable (G, j, cols[j].upper);
						}
						else
	...
	int *hit)
{
	...
	*hit = 0;
	...
	for (i = 0; i < nrows; i++)
	{
		...
		{
			s[i] = mpf_ILL_MAXINT;				/* mpf_ILL_MAXINT means no longer eligible    */
		}
		else
		{
			...
			else if (s[ri] < t0)
			{
				...
				for (k2 = k + 1; k2 < c->deg; k2++)
				{
					...
					if (s[i] == s[ri])
					{
						...
						if (mpf_EGlpNumIsEqual (q, mpf_oneLpNum, ILL_PRE_ZERO_TOL))
						{
							s[ri] = t;
							s[i] = t;
							got++;
	...
	int *hit)
{
	...
	*hit = 0;
	...
	for (j = 0; j < ncols; j++)
	{
		...
		{
			s[j] = mpf_ILL_MAXINT;				/* mpf_ILL_MAXINT means no longer eligible    */
		}
		else
		{
	...
	mpf_t * ub)
{
	...
	for (k = 0; k < rows[i].deg; k++)
	{
		...
		if (e->del == 0)
		{
			if (mpf_EGlpNumIsLessZero (e->coef))
			{
			...
			else if (mpf_EGlpNumIsGreatZero (e->coef))
			{
	...
	mpf_t * ub)
{
	...
	if (mpf_EGlpNumIsLess (ILL_PRE_FEAS_TOL, a_ij->coef))
	{
		if (mpf_EGlpNumIsLess (u, mpf_ILL_MAXDOUBLE))
		{
	...
	else if (mpf_EGlpNumIsLess (a_ij->coef, ILL_PRE_FEAS_TOL))
	{
		if (mpf_EGlpNumIsLess (mpf_ILL_MINDOUBLE, l))
		{
	...
	graph * G)
{
	...
	for (i = 0; i < G->nrows; i++)
	{
		...
		for (k = 0; k < G->rows[i].deg; k++)
		{
			...
			if (G->rows[i].adj[k]->coltype == ILL_PRE_COL_LOGICAL)
				QSlog("S");
			QSlog("(%g)", mpf_EGlpNumToLf (G->rows[i].adj[k]->coef));
		}
		QSlog("  rhs: %g", mpf_EGlpNumToLf (G->rows[i].rhs));
	...
	for (j = 0; j < G->ncols; j++)
	{
		...
		QSlog("  obj: %g  bnd: (%g, %g)", mpf_EGlpNumToLf (G->cols[j].obj),
	...
	mpf_ILLlp_sinfo * s)
{
	...
	if (!sense)
	{
		QSlog("out of memory in mpf_ILLlp_sinfo_print");
		rval = 1;
		goto CLEANUP;
	}
	for (i = 0; i < s->nrows; i++)
