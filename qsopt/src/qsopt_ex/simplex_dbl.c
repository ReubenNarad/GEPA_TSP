...
#include "qsopt_dbl.h"
#include "lib_dbl.h"								/* for dbl_ILLlib_writebasis */
#include "lp_dbl.h"									/* for dbl_ILLwrite_lp */
...
void dbl_ILLsimplex_init_lpinfo (
	dbl_lpinfo * lp)
{
...
void dbl_ILLsimplex_free_lpinfo (
	dbl_lpinfo * lp)
{
	if (lp)
	{
...
void dbl_ILLsimplex_load_lpinfo (
	dbl_ILLlpdata * qslp,
	dbl_lpinfo * lp)
{
	lp->basisid = -1;
	...
	dbl_lpinfo * lp)
{
	...
	if (dbl_EGlpNumIsNeqqZero (lp->tol->ip_tol))
	{
		...
		MESSAGE (VERBOSE_LEVEL, "ip_tol %lg", dbl_EGlpNumToLf (lp->tol->ip_tol));
		MESSAGE (VERBOSE_LEVEL, "eps %lg", dbl_EGlpNumToLf (dbl_epsLpNum));
		MESSAGE (VERBOSE_LEVEL, "dbl_PFEAS_TOLER %lg", dbl_EGlpNumToLf (dbl_PFEAS_TOLER));
#endif
	...
	{
#if VERBOSE_LEVEL <= DEBUG
		MESSAGE (VERBOSE_LEVEL, "id_tol %lg", dbl_EGlpNumToLf (lp->tol->id_tol));
#endif
		dbl_EGlpNumDivUiTo (lp->tol->id_tol, 2UL);
	}
}

void dbl_init_internal_lpinfo (
	dbl_lpinfo * lp)
{
	int rval = 0;
	...
	dbl_lpinfo * lp)
{
	...
	{
		QSlog("dbl_build_internal_lpinfo");
		rval = 1;
		goto CLEANUP;
	}
...
int dbl_ILLsimplex_retest_psolution (
	dbl_lpinfo * lp,
	dbl_price_info * p,
	int phase,
	dbl_feas_info * fi)
...
int dbl_ILLsimplex_retest_dsolution (
	dbl_lpinfo * lp,
	dbl_price_info * p,
	int phase,
	dbl_feas_info * fi)
...
int dbl_ILLsimplex_infcertificate (
	dbl_lpinfo * lp,
	double * pi)
{
	int i, col, nz;
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (sense[i] == 'G' && dbl_EGlpNumIsLessZero (pi[i]))
			dbl_EGlpNumZero (pi[i]);
		if (sense[i] == 'L' && dbl_EGlpNumIsGreatZero (pi[i]))
			dbl_EGlpNumZero (pi[i]);
	}
	return 0;
}
	...
	double * pi)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (lp->O->sense[i] == 'G' && dbl_EGlpNumIsLessZero (pi[i]))
			QSlog("compl");
		if (lp->O->sense[i] == 'L' && dbl_EGlpNumIsGreatZero (pi[i]))
			QSlog("compll");
	}
	...
	itcnt_t*itcnt)
{
	...
	if (it.sdisplay)
	{
		...
		while (i--)
			nonzero += lp->matcnt[i];
		sprintf (buffer, "starting dbl_ILLsimplex on %s...", lp->O->probname);
	...
	if (B != 0)
	{
		...
		{
			QSlog("Unknown algorithm %d in dbl_ILLsimplex", it.algorithm);
			rval = 1;
			ILL_CLEANUP;
		}
	}
...
TERMINATE:
	...
	if (it.sdisplay)
	{
		QSlog("completed dbl_ILLsimplex");
	...
	if (status)
	{
		...
		{
			QSlog("unknown solution status in dbl_ILLsimplex %d",
									it.solstatus);
			rval = 1;
			CHECKRVALG (rval, CLEANUP);
		}
   ...
   /* end update */
	if (it.sdisplay)
	{
		...
		{
		case OPTIMAL:
			QSlog("opt = %f", dbl_EGlpNumToLf (lp->objval));
			break;
		...
		case PRIMAL_FEASIBLE:
			QSlog("primal obj = %f", dbl_EGlpNumToLf (lp->pobjval));
			break;
		...
		case DUAL_UNBOUNDED:
			...
			break;
		case DUAL_FEASIBLE:
			QSlog("dual obj = %f", dbl_EGlpNumToLf (lp->dobjval));
			break;
		}
	...
	double prevobj)
{
	...
	if (!dbl_EGlpNumIsNeqZero (denom, dbl_PROGRESS_THRESH))
	{
	...
	int phase)
{
	...
	case PRIMAL_PHASEI:
		...
		dbl_EGlpNumAddTo (fi.totinfeas, lp->pinfeas);
		strcpy (print_str, "primal infeas");
		if (dbl_EGlpNumIsLessZero (lp->pinfeas) &&
				(dbl_EGlpNumIsNeqZero (lp->pinfeas, dbl_oneLpNum)))
		{
			/*QSlog("Negative Infeasibility! Imposible %lg %la, iter %d",
			 * dbl_EGlpNumToLf (print_val), dbl_EGlpNumToLf (print_val), it->itercnt);
			 */
			//exit(1);
		}
		break;
	...
	case DUAL_PHASEI:
		...
		dbl_EGlpNumAddTo (fi.totinfeas, lp->dinfeas);
	...
	dbl_EGlpNumAddUiTo (fi.totinfeas, 1000);
	if (dbl_EGlpNumIsLessZero (fi.totinfeas))
	{
		...
		if (dbl_EGlpNumIsLess (it->objtol, print_val))
		{
			...
			if (fi.dstatus == DUAL_FEASIBLE)
			{
				...
				if (dbl_EGlpNumIsLess (lp->objbound, lp->dobjval))
				{
					...
					/*if (it->sdisplay) */
					{
						QSlog("bound reached %lf %lf", dbl_EGlpNumToLf (lp->objbound),
										dbl_EGlpNumToLf (lp->dobjval));
					}
				}
				else
	...
	dbl_iter_info * it)
{
	...
	if (pr.price_stat == PRICE_OPTIMAL)
	{
		...
		if (it->sdisplay > 1)
		{
			//ILL_IFTRACE("\t%s:%s:%d\n",__func__,__FILE__,__LINE__);
			QSlog("problem seemingly solved");
			QSlog("seemingly dual opt = %f", dbl_EGlpNumToLf (lp->dobjval));
			QSlog("retesting soln");
		}
	...
	else if (rs.ratio_stat == RATIO_BCHANGE)
	{
		...
		dbl_ILLfct_update_dpII_prices (lp, pinf, &(lp->srhs), &(lp->ssoln), /*rs.eindex,*/
	...
	int *basis_mod)
{
	...
	*basis_mod = 0;
	...
	/* QSlog("Forcing vars into basis in dbl_ILLsimplex_pivotin"); */
	dbl_ILLsvector_init (&wz);
	rval = dbl_ILLsvector_alloc (&wz, lp->nrows);
	CHECKRVALG (rval, CLEANUP);
	dbl_ILLsvector_init (&updz);
	...
	double value)
{
	...
	if (it->sdisplay && it->itercnt % lp->iterskip == 0)
	{
		...
		snprintf (buffer, (size_t) 1023, "(%d): %s = %10.7lf", it->itercnt,
							value_name, dbl_EGlpNumToLf (value));
