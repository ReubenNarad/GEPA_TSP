...
void mpq_ILLfactor_init_factor_work (
	mpq_factor_work * f)
{
...
void mpq_ILLfactor_free_factor_work (
	mpq_factor_work * f)
{
#ifdef UPDATE_STUDY
	if (nupdate)
...
int mpq_ILLfactor_set_factor_iparam (
	mpq_factor_work * f,
	int param,
	int val)
{
	...
	case QS_FACTOR_DENSE_MIN:
		...
		break;
	default:
		QSlog("Invalid param %d in mpq_ILLfactor_set_factor_iparam",
								param);
		return 1;
	}
	return 0;
...
int mpq_ILLfactor_set_factor_dparam (
	mpq_factor_work * f,
	int param,
	mpq_t val)
{
	...
	case QS_FACTOR_PARTIAL_TOL:
		...
		break;
	default:
		QSlog("Invalid param %d in mpq_ILLfactor_set_factor_dparam",
								param);
		return 1;
	}
	return 0;
...
int mpq_ILLfactor_create_factor_work (
	mpq_factor_work * f,
	int dim)
{
	int i;
	...
	int remaining)
{
	...
	for (i = 0; i < dim; i++)
	{
		if (!remaining || ur_inf[i].next >= 0)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				...
				QSlog(" %.3f*%d", mpq_EGlpNumToLf (f->urcoef[beg + j]),
										f->urindx[beg + j]);
				if (f->urcind)
	...
	if (f->dmat)
	{
		...
		for (i = start; i < f->drows; i++)
		{
			...
			for (j = start; j < f->dcols; j++)
			{
				...
				QSlog(" %.3f", mpq_EGlpNumToLf (f->dmat[i * f->dcols + j]));
			}
		}
	}
	...
	if (!remaining)
	{
		for (i = 0; i < f->stage; i++)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				QSlog(" %.3f*%d", mpq_EGlpNumToLf (f->lccoef[beg + j]),
										f->lcindx[beg + j]);
			}
		}
		...
		for (i = 0; i < f->dim; i++)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				QSlog(" %.3f*%d", mpq_EGlpNumToLf (f->lrcoef[beg + j]),
										f->lrindx[beg + j]);
			}
		}
	}
	...
	if (!remaining)
	{
		for (i = 0; i < f->etacnt; i++)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				QSlog(" %.3f*%d", mpq_EGlpNumToLf (f->ercoef[beg + j]),
										f->erindx[beg + j]);
			}
		}
	}
	...
	for (i = 0; i < dim; i++)
	{
		if (!remaining || uc_inf[i].next >= 0)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				if (f->uccoef != 0)
				{
					QSlog(" %.3f*%d", mpq_EGlpNumToLf (f->uccoef[beg + j]),
											f->ucindx[beg + j]);
					if (f->ucrind)
	...
	mpq_factor_work * f)
{
	...
	for (r = 0; r < f->dim; r++)
	{
		...
		for (j = 0; j < nzcnt; j++)
		{
			...
			if (fabs(mpq_EGlpNumToLf(f->uccoef[cbeg + f->urcind[rbeg + j]]) - mpq_EGlpNumToLf(f->urcoef[rbeg + j]))>1000*mpq_EGlpNumToLf(mpq_epsLpNum))
			{
	...
	mpq_t * p_pivot_coef)
{
	...
	for (i = 0; i < prow_nzcnt; i++)
	{
		...
		if (work_indx[j] == 1)
		{
			...
			if ((!(mpq_EGlpNumIsNeqZero (x, f->fzero_tol))) || j == c)
			{
				cancel++;
				if (j != c)
				{
	...
	*p_r = -1;
	for (i = 0; i < nzcnt; i++)
	{
		...
		if((bestnz == -1 || ur_inf[r].pivcnt < bestnz))
		{
			...
			if(mpq_EGlpNumIsLessZero(num_tmp[0]))
...
#ifdef FACTOR_DEBUG
#if (FACTOR_DEBUG+0>1)
	MESSAGE (0,"before Dense mpq_ILLfactor");
	dump_matrix (f, 1);
#endif
#endif
...
#ifdef FACTOR_DEBUG
	MESSAGE (0,"After dense mpq_ILLfactor:\n");
	dump_matrix (f, 0);
#endif /* FACTOR_DEBUG */
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpq_ILLfactor_ftran a after l %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpq_EGlpNumToLf (a[j]));
		}
		MESSAGE(0," ");
#endif
#endif /* SOLVE_DEBUG */
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpq_ILLfactor_ftran a after l:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpq_EGlpNumToLf (a[j]));
	}
...
#ifdef SOLVE_DEBUG
	QSlog("mpq_ILLfactor_ftran x after l3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
#endif /* SOLVE_DEBUG */
}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpq_ILLfactor_ftran a after eta %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpq_EGlpNumToLf (a[j]));
		}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpq_ILLfactor_ftran a after eta:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpq_EGlpNumToLf (a[j]));
	}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpq_ILLfactor_ftran a after eta2 %d:", i);
		for (j = 0; j < anzcnt; j++)
		{
			QSlog(" %.3f*%d", mpq_EGlpNumToLf (acoef[j]), aindx[j]);
		}
#endif
#endif /* SOLVE_DEBUG */
	}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpq_ILLfactor_ftran a after eta2:");
	for (j = 0; j < anzcnt; j++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (acoef[j]), aindx[j]);
	}
	...
	mpq_svector * x)
{
	...
	x->nzcnt = xnzcnt;
#ifdef SOLVE_DEBUG
	QSlog("mpq_ILLfactor_ftran x after u:");
	for (j = 0; j < x->nzcnt; j++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (x->coef[j]), x->indx[j]);
	}
#endif /* SOLVE_DEBUG */
	mpq_EGlpNumClearVar (v);
}
...
#ifdef SOLVE_DEBUG
	QSlog("mpq_ILLfactor_ftran x after u3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
	...
	mpq_svector * x)
{
...
#ifdef RECORD
	{
		...
		for (i = 0; i < a->nzcnt; i++)
		{
			EGioPrintf (fsave, " %d %.16e", a->indx[i], mpq_EGlpNumToLf (a->coef[i]));
		}
		EGioPrintf (fsave, "\n");
		EGioFlush (fsave);
	}
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpq_ILLfactor_ftran a:");
		for (i = 0; i < a->nzcnt; i++)
		{
			QSlog(" %d %la", a->indx[i], mpq_EGlpNumToLf (a->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpq_ILLfactor_ftran x:");
		for (i = 0; i < x->nzcnt; i++)
		{
			QSlog(" %d %la", x->indx[i], mpq_EGlpNumToLf (x->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
	return;
...
/* mpq_ILLfactor_ftran_update solves Bx=a for x, and also returns upd, where Ux=upd */
void mpq_ILLfactor_ftran_update (
	mpq_factor_work * f,
	mpq_svector * a,
	mpq_svector * upd,
	mpq_svector * x)
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpq_ILLfactor_ftran_update a:");
		for (i = 0; i < a->nzcnt; i++)
		{
			QSlog(" %d %.3f", a->indx[i], mpq_EGlpNumToLf (a->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpq_ILLfactor_ftran update x:");
		for (i = 0; i < x->nzcnt; i++)
		{
			QSlog(" %d %.3f", x->indx[i], mpq_EGlpNumToLf (x->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpq_ILLfactor_btran x before l2 %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpq_EGlpNumToLf (x[j]));
		}
#endif
#endif /* SOLVE_DEBUG */
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpq_ILLfactor_btran x after l2:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpq_EGlpNumToLf (x[j]));
	}
...
#ifdef SOLVE_DEBUG
	QSlog("mpq_ILLfactor_btran x after l3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpq_ILLfactor_btran x before eta %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpq_EGlpNumToLf (x[j]));
		}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpq_ILLfactor_btran x after eta:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpq_EGlpNumToLf (x[j]));
	}
	...
	mpq_svector * x)
{
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpq_ILLfactor_btran x before eta2 %d:", i);
		for (j = 0; j < xnzcnt; j++)
		{
			QSlog(" %.3f*%d", mpq_EGlpNumToLf (work_coef[xindx[j]]), xindx[j]);
		}
#endif
#endif /* SOLVE_DEBUG */
	...
	while (j < xnzcnt)
	{
		...
		work_indx[xindx[j]] = 0;
		if (!mpq_EGlpNumIsNeqqZero (xcoef[j]))
		{
			--xnzcnt;
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpq_ILLfactor_btran x after eta2:");
	for (j = 0; j < xnzcnt; j++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (xcoef[j]), xindx[j]);
	}
#endif
#endif /* SOLVE_DEBUG */
	mpq_EGlpNumClearVar (v);
	...
	mpq_svector * x)
{
	...
	x->nzcnt = xnzcnt;
#ifdef SOLVE_DEBUG
	QSlog("mpq_ILLfactor_btran x after u:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
...
#ifdef SOLVE_DEBUG
	QSlog("mpq_ILLfactor_btran x after u3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpq_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpq_ILLfactor_update col %d:", col_p);
		for (i = 0; i < a->nzcnt; i++)
		{
			QSlog(" %.3f*%d", mpq_EGlpNumToLf (a->coef[i]), a->indx[i]);
		}
	}
#endif /* DEBUG_FACTOR */
