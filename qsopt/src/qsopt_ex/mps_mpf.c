	...
	mpf_rawlpdata * lp)
{
	...
	if (skip)
	{
		mpf_ILLmps_set_end_of_line (state);	/* to avoid warning about extra fields */
	}
	else
	{
		if (strcmp (rhsname, " "))
	...
	mpf_qserror_collector * collector)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		...
		{
			str = mpf_EGlpNumGetStr(lp->rhs[i]);
			mpf_ILLprint_report (lp, " RHS    %s    %s\n", rownames[i], str);
			EGfree(str);
		}
	}
	...
	if (lp->rangeval)
	{
		...
		for (i = 0; i < lp->nrows; i++)
		{
			...
			{
				str = mpf_EGlpNumGetStr(lp->rangeval[i]);
				mpf_ILLprint_report (lp, " RANGE    %s    %s\n", rownames[i], str);
				EGfree(str);
			}
		}
	}
	...
	if (ri != lp->nstruct)
	{
		...
		for (; ri < lp->nstruct; ri++)
		{
			...
			{
				str = mpf_EGlpNumGetStr(lp->lower[i]);
				mpf_ILLprint_report (lp, " FX BOUND    %s    %s\n", colnames[ri], str);
				EGfree(str);
				continue;
			}
			...
			if (prtLower)
			{
				...
				{
					str = mpf_EGlpNumGetStr(lp->lower[i]);
					mpf_ILLprint_report (lp, " LO BOUND    %s    %s\n", colnames[ri], str);
					EGfree(str);
				}
			}
			if (prtUpper)
			{
				...
				{
					str = mpf_EGlpNumGetStr(lp->upper[i]);
					mpf_ILLprint_report (lp, " UP BOUND    %s    %s\n", colnames[ri], str);
					EGfree(str);
				}
			}
		}
	...
	char *objname)
{
	...
	{
		str = mpf_EGlpNumGetStr(lp->obj[i]);
		mpf_ILLprint_report (lp, "  %s    %s    %s\n", colname, objname, str);
		EGfree(str);
	}
	for (k = A->matbeg[i]; k < A->matbeg[i] + A->matcnt[i]; k++)
	{
		row = A->matind[k];
		str = mpf_EGlpNumGetStr(A->matval[k]);
