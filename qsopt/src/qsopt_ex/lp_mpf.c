...
/*  EXPORTED FUNCTIONS                                                      */
/*                                                                          */
/*        int mpf_ILLwrite_lp (EGioFile_t *out, mpf_ILLlpdata *lp)                  */
/*        int mpf_ILLread_lp (mpf_qsline_reader f, const char *fname, mpf_rawlpdata *lp)*/
	...
	mpf_qserror_collector * collector)
{
	...
	/* Note, mpf_ILLlp_rows_init returns cols ordered by structmap, so we may use 
	...
	for (ri = 0, var = 0; ri < lp->nstruct; ri++)
	{
		...
		if (mpf_EGlpNumIsNeqqZero (lp->obj[i]))
		{
			...
			if ((line->total >= LINE_LEN) && (var >= 4))
			{
				...
				while (k < lp->nstruct)
				{
					if (mpf_EGlpNumIsLessZero (lp->obj[lp->structmap[k]]))
					{
						break;
					}
					else
					{
						if (mpf_EGlpNumIsGreatZero (lp->obj[lp->structmap[k]]))
						{
							mpf_ILLwrite_lp_state_append (line, " +");
							break;
						}
		...
		if (colInRow[i] == row)
		{
			if (mpf_EGlpNumIsNeqqZero (colCoef[i]))
			{
				...
				if (line->total >= LINE_LEN)
				{
					mpf_ILLprint_report (lp, "%s\n", line->buf);
					mpf_ILLwrite_lp_state_start (line);
					if ((!firstVar) && !mpf_EGlpNumIsLessZero (*coef))
					{
						mpf_ILLwrite_lp_state_append (line, " +");
					}
					var = 0;
	...
	char **colnames)
{
	...
	if (ri != lp->nstruct)
	{
		...
		for (; ri < lp->nstruct; ri++)
		{
			...
			prtUpper = !mpf_ILLraw_default_upper (lp, i, ri);
 ...
 */
	...
	rval = mpf_ILLread_lp_state_init (state, file, fname, 0);
	CHECKRVALG (rval, CLEANUP);
...
int mpf_ILLread_constraint_expr (
	mpf_ILLread_lp_state * state,
	mpf_rawlpdata * lp,
	int rowind,
	int allowNew)
...
int mpf_ILLread_constraint_name (
	mpf_ILLread_lp_state * state,
	char **rowname)
{
	int rval = 0;
...
int mpf_ILLread_one_constraint (
	mpf_ILLread_lp_state * state,
	const char *rowname,
	mpf_rawlpdata * lp,
	int allowNewCols)
