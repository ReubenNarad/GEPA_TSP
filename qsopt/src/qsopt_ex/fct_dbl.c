...
dbl_bndinfo *dbl_ILLfct_new_bndinfo (
	void)
{
...
void dbl_ILLfct_load_workvector (
	dbl_lpinfo * lp,
	dbl_svector * s)
{
	int i;
...
void dbl_ILLfct_zero_workvector (
	dbl_lpinfo * lp)
{
	int i;
...
void dbl_ILLfct_set_variable_type (
	dbl_lpinfo * lp)
{
	int j;
	...
	for (j = 0; j < lp->ncols; j++)
	{
		...
		case 0:
			if (dbl_EGlpNumIsLess (lp->lz[j], lp->uz[j]))
				lp->vtype[j] = VBOUNDED;
			else if (!dbl_EGlpNumIsNeqqZero (lp->lz[j]) &&
							 (lp->vclass[j] == CLASS_LOGICAL))
...
void dbl_ILLfct_compute_phaseI_xbz (
	dbl_lpinfo * lp)
{
	int i, j, r;
	int col, mcnt, mbeg;
...
void dbl_ILLfct_compute_phaseI_piz (
	dbl_lpinfo * lp)
{
	int i, r;
	dbl_svector *srhs = &(lp->srhs);
...
void dbl_ILLfct_compute_phaseI_dz (
	dbl_lpinfo * lp)
{
	int i, j;
	int col;
...
void dbl_ILLfct_compute_binvrow (
	dbl_lpinfo * lp,
	dbl_svector * zz,
	int row,
	double ztoler)
{
	...
	if (dbl_EGlpNumIsGreatZero (ztoler))
...
void dbl_ILLfct_compute_psteep_upv (
	dbl_lpinfo * lp,
	dbl_svector * swz)
{
...
void dbl_ILLfct_compute_dsteep_upv (
	dbl_lpinfo * lp,
	dbl_svector * swz)
{
...
int dbl_ILLfct_compute_zA (
	dbl_lpinfo * lp,
	dbl_svector * z,
	dbl_svector * zA)
{
	if (z->nzcnt < lp->nrows / 2)
		return compute_zA3 (lp, z, zA, dbl_PIVZ_TOLER);
	else
		return compute_zA1 (lp, z, zA, dbl_PIVZ_TOLER);
}

/* compute v^T A */
void dbl_ILLfct_compute_vA (
	dbl_lpinfo * lp,
	dbl_svector * v,
	double * vA)
{
	...
	for (j = 0; j < lp->ncols; j++)
		if (!dbl_EGlpNumIsNeqZero (vA[j], dbl_SZERO_TOLER))
			dbl_EGlpNumZero (vA[j]);
...
*/
void dbl_ILLfct_update_basis_info (
	dbl_lpinfo * lp,
	int eindex,
	int lindex,
	int lvstat)
	...
	const double alpha)
{
	...
	if (!dbl_EGlpNumIsNeqqZero (alpha))
		return;
	...
	int *chgb)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		...
		{
			dbl_EGlpNumSet (newb, -1.0 * (my_rand (50, &(lp->rstate)) + 1.0));
		...
		{
			dbl_EGlpNumSet (newb, my_rand (50, &(lp->rstate)) + 1.0);
			dbl_EGlpNumMultTo (newb, cftol);
...
int dbl_ILLfct_adjust_viol_bounds (
	dbl_lpinfo * lp)
{
	int rval = 0;
	int chgb = 0;
...
int dbl_ILLfct_perturb_phaseI_bounds (
	dbl_lpinfo * lp)
{
	int rval = 0;
	int chgb = 0;
...
void dbl_ILLfct_unroll_bound_change (
	dbl_lpinfo * lp)
{
	int col;
	int changex = 0;
	...
	while (lp->nbchange != 0)
	{
		...
		if (lp->vtype[col] == VBOUNDED)
		{
			if (dbl_EGlpNumIsEqqual (lp->lz[col], lp->uz[col]))
				lp->vtype[col] = (!dbl_EGlpNumIsNeqqZero (lp->lz[col])) ?
					VARTIFICIAL : VFIXED;
		}
	...
	int *chgc)
{
	...
	for (i = 0; i < lp->nnbasic; i++)
	{
		...
		case STAT_LOWER:
			...
			{
				dbl_EGlpNumSet (newc, my_rand (50, &(lp->rstate)) + 1.0);
				dbl_EGlpNumMultTo (newc, cftol);
				dbl_EGlpNumAddTo (newc, *c);
				if (dbl_EGlpNumIsLessZero (*dj))
...
int dbl_ILLfct_adjust_viol_coefs (
	dbl_lpinfo * lp)
{
	int rval = 0;
	int chgc = 0;
...
void dbl_ILLfct_unroll_coef_change (
	dbl_lpinfo * lp)
{
	int bascoef = 0;
...
void dbl_ILLfct_check_pfeasible (
	dbl_lpinfo * lp,
	dbl_feas_info * fs,
	const double ftol)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
				...
				&& dbl_EGlpNumIsNeq (lp->uz[col], dbl_INFTY, dbl_oneLpNum))
		{
			...
			WARNINGL (QSE_WLVL, dbl_EGlpNumIsLess (dbl_INFTY, err1),
						 ...
						 && dbl_EGlpNumIsNeq (lp->lz[col], dbl_NINFTY, dbl_oneLpNum))
		{
			...
			WARNINGL (QSE_WLVL, dbl_EGlpNumIsLess (dbl_INFTY, err2),
	...
	if (dbl_EGlpNumIsNeqqZero (infeas))
	{
		...
		if (dbl_EGlpNumIsLessZero (fs->totinfeas))
		{
...
/* feasibility routines */
void dbl_ILLfct_check_pIpfeasible (
	dbl_lpinfo * lp,
	dbl_feas_info * fs,
	double ftol)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (!dbl_EGlpNumIsNeqZero (lp->xbz[i], ftol))
			continue;
		col = lp->baz[i];
		if (dbl_EGlpNumIsGreatZero(lp->xbz[i]) &&
				dbl_EGlpNumIsNeqq (lp->uz[col], dbl_INFTY))
		{
			ninf++;
		}
		else if (dbl_EGlpNumIsLessZero (lp->xbz[i]) &&
...
void dbl_ILLfct_check_dfeasible (
	dbl_lpinfo * lp,
	dbl_feas_info * fs,
	const double ftol)
{
	...
	{
		lp->dfeas[j] = 0;
		if (!dbl_EGlpNumIsNeqZero (lp->dz[j], ftol))
			continue;
		col = lp->nbaz[j];
...
void dbl_ILLfct_check_pIdfeasible (
	dbl_lpinfo * lp,
	dbl_feas_info * fs,
	double ftol)
{
	...
	for (j = 0; j < lp->nnbasic; j++)
	{
		if (!dbl_EGlpNumIsNeqZero (dz[j], ftol))
			continue;
		col = lp->nbaz[j];
		if (lp->vtype[col] == VARTIFICIAL || lp->vtype[col] == VFIXED)
			continue;

		if (dbl_EGlpNumIsLessZero (dz[j]) &&
				(lp->vstat[col] == STAT_LOWER || lp->vstat[col] == STAT_ZERO))
			ninf++;
		else if (dbl_EGlpNumIsGreatZero (dz[j]) &&
						 (lp->vstat[col] == STAT_UPPER || lp->vstat[col] == STAT_ZERO))
			ninf++;
	}
...
void dbl_ILLfct_dphaseI_simple_update (
	dbl_lpinfo * lp,
	double ftol)
{
	int j, col;
	...
	for (j = 0; j < lp->nnbasic; j++)
	{
		if (!dbl_EGlpNumIsNeqZero (lp->dz[j], ftol))
			continue;
		col = lp->nbaz[j];
		if (dbl_EGlpNumIsLessZero (lp->dz[j] ) && lp->vtype[col] == VBOUNDED)
			lp->vstat[col] = STAT_UPPER;
		else if (dbl_EGlpNumIsGreatZero (lp->dz[j]) && lp->vtype[col] == VBOUNDED)
			lp->vstat[col] = STAT_LOWER;
	}
}

/* set status values */
void dbl_ILLfct_set_status_values (
	dbl_lpinfo * lp,
	int pstatus,
	int dstatus,
	int ptype,
...
#define C_VALUE(a) (1.0+(double)(a)/(PARAM_HEAP_RATIO*ILLutil_our_log2(a)))
	dbl_EGlpNumSet (c->y_ravg, C_VALUE (lp->nrows));
	dbl_EGlpNumSet (c->za_ravg, C_VALUE (lp->nnbasic));
...
void dbl_ILLfct_update_ppI_prices (
	dbl_lpinfo * lp,
	dbl_price_info * pinf,
	dbl_svector * srhs,
	dbl_svector * ssoln,
	...
	dbl_svector * srhs)
{
	...
	for (j = 0; j < tctr && dbl_EGlpNumIsLeq (t[perm[j]], tz); j++)
	{
		...
		if (lp->iwork[c] != 1)
		{
			...
			if (cbnd == BSKIP)
			{
				if (!dbl_EGlpNumIsNeqZero (dj, *dftol));
				else if (dbl_EGlpNumIsLessZero (dj) && vs == STAT_LOWER)
					lp->vstat[col] = STAT_UPPER;
				else if (dbl_EGlpNumIsGreatZero (dj) && vs == STAT_UPPER)
					lp->vstat[col] = STAT_LOWER;
			}
			else if (c != eindex)
			{
				if (!dbl_EGlpNumIsNeqZero (dj, *dftol))
					f = 0;
				else if (dbl_EGlpNumIsLessZero (dj) &&
								 (vs == STAT_LOWER || vs == STAT_ZERO))
					f = -1;
				else if (dbl_EGlpNumIsGreatZero (dj) &&
...
void dbl_ILLfct_update_dpI_prices (
	dbl_lpinfo * lp,
	dbl_price_info * pinf,
	dbl_svector * srhs,
	dbl_svector * ssoln,
...
void dbl_ILLfct_update_dIIfeas (
	dbl_lpinfo * lp,
	int eindex,
	dbl_svector * srhs)
{
...
void dbl_ILLfct_update_dpII_prices (
	dbl_lpinfo * lp,
	dbl_price_info * pinf,
	dbl_svector * srhs,
	dbl_svector * ssoln,
	...
	double alpha)
{
	...
	else
	{
		...
		dbl_ILLfct_update_xz (lp, alpha, -1, -1);
		add_vectors (lp, ssoln, &(lp->yjz), ssoln, dbl_oneLpNum);
		u = ssoln;
	}
	...
	dbl_price_info * p)
{
	...
	if (dbl_EGlpNumIsNeqqZero (err))
		QSlog("dI x err = %.7f, ern = %d", dbl_EGlpNumToLf (err), ern);
	...
	dbl_price_info * p)
{
	...
	if (dbl_EGlpNumIsNeqqZero (err))
		QSlog("dII x err = %.7f, ern = %d", dbl_EGlpNumToLf (err), ern);
	...
	dbl_price_info * p)
{
	...
	if (dbl_EGlpNumIsNeqqZero (err))
		QSlog("pI pi err = %.7f, ern = %d", dbl_EGlpNumToLf (err), ern);
	...
	if (dbl_EGlpNumIsNeqqZero (err))
		QSlog("pI dz err = %.7f, ern = %d", dbl_EGlpNumToLf (err), ern);
	...
	dbl_price_info * p)
{
	...
	if (dbl_EGlpNumIsNeqqZero (err))
		QSlog("pII pi err = %.7f, ern = %d", dbl_EGlpNumToLf (err), ern);
	...
	if (dbl_EGlpNumIsNeqqZero (err))
		QSlog("pII dz err = %.7f, ern = %d", dbl_EGlpNumToLf (err), ern);
	/*
