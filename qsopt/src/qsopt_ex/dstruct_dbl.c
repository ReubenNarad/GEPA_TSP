...
int dbl_ILLsvector_copy (
	const dbl_svector * s_in,
	dbl_svector * s_out)
{
	int i;
	...
	dbl_heap * h)
{
	...
	QSlog("\n key: ");
	for (i = 0; i < h->maxsize; i++)
		QSlog("%la ", dbl_EGlpNumToLf (h->key[i]));
	QSlog("\n key(sorted): ");
	for (i = 0; i < h->size; i++)
		QSlog("%la ", dbl_EGlpNumToLf (h->key[h->entry[i]]));
}
	...
	dbl_heap * h)
{
	...
	{
		if (h->loc[i] < -1)
			QSlog("error in dbl_heap\n");
		else if (h->loc[i] > -1)
			tcnt++;
	}
	if (tcnt != h->size)
		QSlog("error 3 in dbl_heap\n");

	for (i = 0; i < h->size; i++)
	{
		if (h->loc[h->entry[i]] != i)
			QSlog("error 1 in dbl_heap\n");
		if (!dbl_EGlpNumIsNeqqZero (h->key[h->entry[i]]))
			QSlog("error 2 in dbl_heap\n");
		if (dbl_EGlpNumIsLess (h->key[h->entry[HEAP_UP (i)]], h->key[h->entry[i]]))
			QSlog("error 4 in dbl_heap\n");
	}
}

#endif

void dbl_ILLheap_insert (
	dbl_heap * const h,
	int const ix)
{
	int i = h->size;
	...
	double * key)
{
	...
	for (i = 0; i < nelems; i++)
	{
		if (dbl_EGlpNumIsGreatZero (key[i]))
		{
			h->entry[n] = i;
			h->loc[i] = n;
			n++;
	...
	dbl_ILLmatrix * A)
{
	...
	else
	{
		...
		for (j = 0; j < A->matcols; j++)
		{
			...
			{
				EGioPrintf (fd, "row %d=%.3f ", A->matind[k], dbl_EGlpNumToLf (A->matval[k]));
			}
			EGioPrintf (fd, "\n");
		}
	}
