...
//#define DJZERO_TOLER mpq_PFEAS_TOLER
...
void mpq_ILLbasis_init_basisinfo (
	mpq_lpinfo * lp)
{
	lp->baz = 0;
	lp->nbaz = 0;
...
void mpq_ILLbasis_free_basisinfo (
	mpq_lpinfo * lp)
{
	...
	int strict)
{
	...
	if (lp->basisid == -1)
	{
		QSlog("mpq_ILLbasis_tableau_row: no basis");
	...
	if (rhs != NULL && trow != NULL)
	{
		...
		for (i = 0; i < lp->nrows; i++)
			if (mpq_EGlpNumIsGreatZero (tr[lp->baz[i]]))
		...
		if (mpq_EGlpNumIsLessZero (coef))
			mpq_EGlpNumSign (coef);
		if (mpq_EGlpNumIsLess (mpq_PIVZ_TOLER, coef))
			QSlog("tableau: bas computed = %.12f", mpq_EGlpNumToLf (sum));
		if (!strict)
	...
	int *vstat)
{
	...
	get_var_info (lp, &vd);
	if (!mpq_EGlpNumIsNeqqZero (vd.cmax))
		mpq_EGlpNumOne (cmax);
	else
	{
	...
	int *vstat)
{
	...
	for (k = 0; k < s_i; k++)
	{
		...
		for (j = 0; j < rcnt; j++)
		{
			...
			if (rcol[col] == 1)
				break;
			if (mpq_EGlpNumIsLessZero (dj[col]))
			{
 ...
 * mpq_CB_EPS = 0.001;
 * mpq_CB_PRI_RLIMIT = 0.25;
 * mpq_CB_INF_RATIO = 10.0; 
 * */
	int choice = 1;
	mpq_t rp, rd;
...
#if BASIS_STATS > 0
	QSlog("b1: nz %d pinf %.2f dinf %.2f", nz1, mpq_EGlpNumToLf (pinf1),
							mpq_EGlpNumToLf (dinf1));
	QSlog("b2: nz %d pinf %.2f dinf %.2f", nz2, mpq_EGlpNumToLf (pinf2),
	...
	int *singular)
{
	...
	*singular = 0;
	do
	{
		...
		rval = mpq_ILLfactor_create_factor_work (lp->f, lp->O->nrows);
		CHECKRVALG (rval, CLEANUP);
...
void mpq_ILLbasis_column_solve_update (
	mpq_lpinfo * lp,
	mpq_svector * rhs,
	mpq_svector * upd,
	mpq_svector * soln)
...
/* Bico - comment out for dist
       QSlog("Warning: numerically bad basis in mpq_ILLfactor_update");
*/
		*refactor = 1;
		rval = 0;
	}
