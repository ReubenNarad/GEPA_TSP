...
#define ILL_PRE_FEAS_TOL dbl_PFEAS_TOLER	//(1e-6)
#define ILL_PRE_ZERO_TOL dbl_PIVOT_TOLER	//(1e-10)
	...
	dbl_ILLlpdata * lp)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlp_add_logicals called with a NULL pointer");
		rval = 1;
		goto CLEANUP;
	}

	QSlog("dbl_ILLlp_add_logicals ...");
	...
	for (i = 0; i < nrows; i++)
	{
		...
		case 'R':									/* Range     */
			...
			break;
		default:
			QSlog("unknown sense %c in dbl_ILLlp_add_logicals", sense[i]);
			rval = 1;
			goto CLEANUP;
		}
		ncols++;
	...
	int pre_types)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlp_presolve called with a NULL pointer");
		rval = 1;
		goto CLEANUP;
	}
	...
	dbl_ILLlp_preline * line)
{
	...
	if (line->row_or_col == 0)
	{
		...
		{
			QSlog(" C%d->%g", line->ind[k], dbl_EGlpNumToLf (line->val[k]));
		}
		QSlog(" RHS->%g", dbl_EGlpNumToLf (line->rhs));
	}
	else
	{
		for (k = 0; k < line->count; k++)
		{
			QSlog(" R%d->%g", line->ind[k], dbl_EGlpNumToLf (line->val[k]));
		}
		QSlog(" Obj->%g  LB->%g  UB->%g", dbl_EGlpNumToLf (line->obj),
	...
	for (j = 0; j < ncols; j++)
	{
		if (cols[j].del == 0)
		{
			...
			if (k == cols[j].deg)
			{
				...
				if (!dbl_EGlpNumIsNeqZero (objtmp, ILL_PRE_FEAS_TOL))
				{
					set_fixed_variable (G, j, cols[j].lower);
				}
				else if (dbl_EGlpNumIsGreatZero (objtmp))
				{
					if (dbl_EGlpNumIsEqqual (cols[j].lower, dbl_ILL_MINDOUBLE))
					{
						...
						QSlog("col %d, obj %g", j, dbl_EGlpNumToLf (cols[j].obj));
						rval = 1;
						goto CLEANUP;
					}
					else
	...
	int *hit)
{
	...
	*hit = 0;
	...
	while (list)
	{
		...
		if (tdeg[rowindex] == 0)
		{
			if (dbl_EGlpNumIsNeqZero (r->rhs, ILL_PRE_FEAS_TOL))
			{
				QSlog("infeasible row detected in singleton_row");
				QSlog("empty row with rhs = %g", dbl_EGlpNumToLf (r->rhs));
				rval = 1;
				goto CLEANUP;
			}
		...
		else
		{
			...
			if (dbl_EGlpNumIsSumLess (val, ILL_PRE_FEAS_TOL, c->lower) ||
					dbl_EGlpNumIsSumLess (c->upper, ILL_PRE_FEAS_TOL, val))
			{
	...
	*hit = 0;
	...
	for (i = 0; i < nrows; i++)
	{
		if (rows[i].del == 0)
		{
			...
			if (dbl_EGlpNumIsSumLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ||
					dbl_EGlpNumIsSumLess (ub, ILL_PRE_FEAS_TOL, rows[i].rhs))
			{
			...
			else if (dbl_EGlpNumIsDiffLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ||
							 dbl_EGlpNumIsDiffLess (ub, ILL_PRE_FEAS_TOL, rows[i].rhs))
			{
				(*hit)++;
				ts = (dbl_EGlpNumIsDiffLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ? 0 : 1);
				for (k = 0; k < rows[i].deg; k++)
				{
					e = rows[i].adj[k];
					if (e->del == 0)
			...
			else if (s[ri] < t0)
			{
				...
				for (k2 = k + 1; k2 < c->deg; k2++)
				{
					...
					if (s[i] == s[ri])
					{
						...
						if (dbl_EGlpNumIsEqual (q, dbl_oneLpNum, ILL_PRE_ZERO_TOL))
						{
							s[ri] = t;
							s[i] = t;
							got++;
	...
	double * ub)
{
	...
	if (dbl_EGlpNumIsLess (ILL_PRE_FEAS_TOL, a_ij->coef))
	{
		if (dbl_EGlpNumIsLess (u, dbl_ILL_MAXDOUBLE))
		{
	...
	else if (dbl_EGlpNumIsLess (a_ij->coef, ILL_PRE_FEAS_TOL))
	{
		if (dbl_EGlpNumIsLess (dbl_ILL_MINDOUBLE, l))
		{
	...
	graph * G)
{
	...
	for (i = 0; i < G->nrows; i++)
	{
		...
		for (k = 0; k < G->rows[i].deg; k++)
		{
			...
			if (G->rows[i].adj[k]->coltype == ILL_PRE_COL_LOGICAL)
				QSlog("S");
			QSlog("(%g)", dbl_EGlpNumToLf (G->rows[i].adj[k]->coef));
		}
		QSlog("  rhs: %g", dbl_EGlpNumToLf (G->rows[i].rhs));
	...
	for (j = 0; j < G->ncols; j++)
	{
		...
		QSlog("  obj: %g  bnd: (%g, %g)", dbl_EGlpNumToLf (G->cols[j].obj),
	...
	dbl_ILLlp_sinfo * s)
{
	...
	if (!sense)
	{
		QSlog("out of memory in dbl_ILLlp_sinfo_print");
		rval = 1;
		goto CLEANUP;
	}
	for (i = 0; i < s->nrows; i++)
