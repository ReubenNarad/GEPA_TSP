...
void mpq_ILLutil_dheap_findmin (
	mpq_ILLdheap * h,
	int *i)
{
	if (h->size == 0)
...
int mpq_ILLutil_dheap_insert (
	mpq_ILLdheap * h,
	int i)
{
	if (h->size >= h->total_space)
	{
		QSlog("Error - mpq_heap already full");
		return 1;
	}
	h->size++;
...
void mpq_ILLutil_dheap_deletemin (
	mpq_ILLdheap * h,
	int *i)
{
	int j;
...
void mpq_ILLutil_dheap_changekey (
	mpq_ILLdheap * h,
	int i,
	mpq_t * newkey)
{
