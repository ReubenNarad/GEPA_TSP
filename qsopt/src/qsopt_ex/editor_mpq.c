...
static int transpose ( mpq_rawlpdata * lp);
static int pull_info_from_p ( mpq_QSdata * p, mpq_rawlpdata * lp);
static void add_row ( mpq_QSdata * p, mpq_rawlpdata * lp, mpq_ILLread_lp_state * state);

/* static int new_row(mpq_QSdata *p, mpq_rawlpdata *lp, mpq_ILLread_lp_state *state); */
static void del_row ( mpq_QSdata * p, mpq_rawlpdata * lp, mpq_ILLread_lp_state * state);

static void add_col ( mpq_QSdata * p, mpq_rawlpdata * lp, mpq_ILLread_lp_state * state);
static void del_col ( mpq_QSdata * p, mpq_rawlpdata * lp, mpq_ILLread_lp_state * state);
	...
	int *subcmd)
{
	...
	*subcmd = -1;
	if (hasSubCmd[*cmd] && (mpq_ILLread_lp_state_next_field_on_line (state) == 0))
	{
		...
		case PRTX:
			{
				...
				if ((rval = mpq_ILLlib_print_x (lout, p->lp, 0, 0, 1)))
				{
	...
	int salgo)
{
	...
	if (p->simplex_display)
		if (rval == 0)
		{
			QSlog("LP Value: %.6f, status %d", mpq_EGlpNumToLf (val),
									status);
		}
CLEANUP:
