...
/*  EXPORTED FUNCTIONS                                                      */
/*                                                                          */
/*    int dbl_ILLlib_optimize (dbl_lpinfo *lp, dbl_ILLlp_basis *B, dbl_price_info *pinf,    */
...
/*    int dbl_ILLlib_newrows (dbl_lpinfo *lp, dbl_ILLlp_basis *B, int num, double *rhs, */
...
/*    int dbl_ILLlib_addrow (dbl_lpinfo *lp, dbl_ILLlp_basis *B, int cnt, int *ind,     */
...
/*            const char **names, int factorok)                             */
/*    int dbl_ILLlib_delcols (dbl_lpinfo *lp, dbl_ILLlp_basis *B, int num, int *dellist */
/*            int *basis_ok)                                                */
...
/*    int dbl_ILLlib_getrows (dbl_lpinfo *lp, int num, int *rowlist, int **rowcnt,  */
/*            int **rowbeg, int **rowind, double **rowval, double **rhs,    */
/*            char **sense, char ***names)                                  */
/*    int dbl_ILLlib_getcols (dbl_lpinfo *lp, int num, int *collist, int **colcnt,  */
...
/*    int dbl_ILLlib_readbasis (dbl_lpinfo *lp, dbl_ILLlp_basis *B, char *fname)        */
/*    int dbl_ILLlib_writebasis (dbl_lpinfo *lp, const char *fname)                 */
...
/*    int dbl_ILLlib_print_x (EGioFile_t *fd, dbl_lpinfo *lp, dbl_ILLlp_cache *C, double *x,  */
/*            int nonZerosOnly)                                             */
	...
	itcnt_t*itcnt)
{
	...
	/* dbl_ILLprice_free_pricing_info (pinf); *//* Should be removed later */

	rval = dbl_ILLsimplex (lp, algo, B, pinf, &sol_status, simplex_display, itcnt);
	...
	double * rc)
{
	...
	if (C)
	{
		...
		{
			QSlog("cache mismatch in dbl_ILLlib_solution");
			rval = 0;
			ILL_CLEANUP;
		}
		if (val)
	...
	double * tabrow)
{
	...
	{
		QSlog("dbl_ILLlib_tableau called with bad row: %d", row);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	const double bnd)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_chgbnd called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	{
		QSlog("dbl_ILLlib_chgbnd called with bad indx: %d", indx);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	case 'B':
		...
		break;
	default:
		QSlog("dbl_ILLlib_chgbnd called with lu: %c", lu);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	double * bnd)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getbnd called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (indx < 0 || indx > lp->O->nstruct)
	{
		QSlog("dbl_ILLlib_getbnd called with bad indx: %d", indx);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	case 'U':
		...
		break;
	default:
		QSlog("dbl_ILLlib_getbnd called with lu: %c", lu);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	double *upper)
{
    ...
    if (!lp) {
        QSlog("dbl_ILLlib_getbnds_list called without an lp");
        rval = 1; ILL_CLEANUP;
    }
    ...
    nstruct = qslp->nstruct;
		...
		if(collist[j]<0|| collist[j] >= nstruct)
			{
				QSlog("dbl_ILLlib_getbnds_list collist[%d] = %d out "
	...
	itcnt_t*itcnt)
{
	...
	{
		lp->basisid = -1;
		rval = dbl_ILLlib_optimize (lp, 0, &lpinf, DUAL_SIMPLEX, &status, 0, itcnt);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < ncand; i++)
	{
		...
		rval = dbl_ILLlib_optimize (lp, &B, &lpinf, DUAL_SIMPLEX, &status, 0, itcnt);
		CHECKRVALG (rval, CLEANUP);
	...
	lp->maxiter = olditer;
	rval = dbl_ILLlib_optimize (lp, &origB, pinf, DUAL_SIMPLEX, &status, 0, itcnt);
	CHECKRVALG (rval, CLEANUP);
	...
	const char *name)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_addrow called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	{
		dbl_EGlpNumReallocArray (&(qslp->rhs), qslp->rowsize + EXTRA_ROWS);
		...
		if (qslp->rangeval || sense == 'R')
			dbl_EGlpNumReallocArray (&(qslp->rangeval), qslp->rowsize + EXTRA_ROWS);
	...
	{
		dbl_EGlpNumReallocArray (&(qslp->lower), qslp->colsize + EXTRA_COLS);
		dbl_EGlpNumReallocArray (&(qslp->upper), qslp->colsize + EXTRA_COLS);
		dbl_EGlpNumReallocArray (&(qslp->obj), qslp->colsize + EXTRA_COLS);
	...
	int *cache_ok)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_delrows called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
...
*/
		...
		if (bok == 1)
		{
			...
			if (B->rownorms)
			{
				...
				if (k != nrows - num)
				{
					QSlog("error in  dbl_ILLlib_delrows");
					rval = 1;
					ILL_CLEANUP;
				}
			}
	...
	int *basis_ok)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_delcols called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	double*val)
{
	...
	for (i = A->matbeg[col]; i < A->matbeg[col] + A->matcnt[col]; i++)
	{
		if (A->matind[i] == row)
		{
			dbl_EGlpNumCopy(*val, A->matval[i]);
			ILL_CLEANUP;
		}
	}
...
{ 
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_chgcoef called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	{
		QSlog("dbl_ILLlib_getcoef called with out-of-range index");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	double coef)
{
	...
	if (rowindex < 0 || rowindex >= nrows || colindex < 0 || colindex >= nstruct)
	{
		QSlog("dbl_ILLlib_chgcoef called with out-of-range index");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	char *sense)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		case 'R':									/* Range constraint, we will set its upper bound
																 once we call dbl_QSchange_range, by default it 
		...
		case 'L':									/* Slack     */
			...
			break;
		default:
			QSlog("illegal sense %c in dbl_ILLlib_chgsense", sense[i]);
			rval = 1;
			ILL_CLEANUP;
		}
	}
	...
	int factorok)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_addcol called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	const double * rowval)
{
	...
	newval = dbl_EGlpNumAllocArray (A->matsize + rowcnt + EXTRA_MAT);
	...
	double * colval)
{
	...
	if (A->matfree < colcnt + 1)
	{
		...
		//                                 sizeof (int));
		//CHECKRVALG(rval,CLEANUP);
		dbl_EGlpNumReallocArray (&(A->matval), A->matsize + colcnt + EXTRA_MAT + 1);
	...
	char ***names)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getrows called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (range)
	{
		*range = dbl_EGlpNumAllocArray(num);
		if(qslp->rangeval)
		{
			for(i = 0; i < num ; i++)
			{
				dbl_EGlpNumCopy((*range)[i], qslp->rangeval[rowlist[i]]);
			}
		}
		else
		{
	...
	char ***names)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getcols called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	double* obj)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getobj_list called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	for (j = 0; j < num; j++)
	{
		...
		{
			QSlog("dbl_ILLlib_getobj_list collist[%d] = %d outside"
									" valid range", j, col);
			rval = 1;
			ILL_CLEANUP;
		}
		dbl_EGlpNumCopy(obj[j],qslp->obj[structmap[col]]);
	}  
	...
	double * obj)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getobj called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	for (j = 0; j < nstruct; j++)
	{
		dbl_EGlpNumCopy (obj[j], qslp->obj[structmap[j]]);
	}
	...
	double coef)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_chgobj called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (indx < 0 || indx >= lp->O->nstruct)
	{
		QSlog("dbl_ILLlib_chgrhs called with bad indx: %d", indx);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	double * rhs)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getrhs called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	double coef)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_chgrhs called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	dbl_EGlpNumCopy(qslp->rangeval[indx], coef);
	...
	char **rownames)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_rownames called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	if (!rownames)
	{
		QSlog("dbl_ILLlib_rownames called with NULL rownames");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	int *intflags)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getintflags called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	char **colnames)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_colnames called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	if (!colnames)
	{
		QSlog("dbl_ILLlib_colnames called with NULL colnames");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	int *colindex)
{
	...
	*colindex = -1;
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_colindex called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	int *rowindex)
{
	...
	*rowindex = -1;
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_rowindex called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	char *rstat)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_getbasis called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (lp->basisid == -1)
	{
		QSlog("dbl_ILLlib_getbasis called with modifed LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	for (i = 0; i < qslp->nstruct; i++)
	{
		...
		case STAT_ZERO:
			...
			break;
		default:
			QSlog("unknown vstat in dbl_ILLlib_getbasis: %d", lp->vstat[j]);
			rval = 1;
			ILL_CLEANUP;
		}
	}
	...
	for (i = 0; i < nrows; i++)
	{
		...
		if (qslp->rangeval && dbl_EGlpNumIsNeqqZero (qslp->rangeval[i]))
		{
			...
			case STAT_UPPER:
				...
				break;
			default:
				QSlog("unknown vstat in dbl_ILLlib_getbasis 2");
				rval = 1;
				ILL_CLEANUP;
			}
		}
		else
		{
			...
			case STAT_LOWER:
				...
				break;
			default:
				QSlog("unknown vstat in dbl_ILLlib_getbasis 3: %d, %d",
										i, lp->vstat[j]);
				rval = 1;
				ILL_CLEANUP;
			}
	...
	const char *fname)
{
	...
	in = dbl_ILLline_reader_new ((dbl_qsread_line_fct) EGioGets, file_in);
	rval = dbl_ILLmps_state_init (&state, in, fname);
	CHECKRVALG (rval, CLEANUP);
	...
	while (dbl_ILLmps_next_line (&state) == 0)
	{
		...
		else
		{
			...
			sec = ILLutil_index (dbl_ILLmps_section_name, state.key);
	...
	const char *fname)
{
	...
	if (!lp)
	{
		QSlog("dbl_ILLlib_writebasis called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	if (!B && lp->basisid == -1)
	{
		QSlog("dbl_ILLlib_writebasis called with unsolved LP");
		rval = 1;
		ILL_CLEANUP;
	}
...
*/
	...
	if (basic != nrows)
	{
		QSlog("error in dbl_ILLlib_getrownorms");
		rval = 1;
		ILL_CLEANUP;
	}
...
int dbl_ILLlib_recompute_rownorms (
	dbl_lpinfo * lp,
	dbl_price_info * pinf)
{
	int rval = 0;
...
#define PRINT_TOL dbl_PFEAS_TOLER

int dbl_ILLlib_print_x (
	EGioFile_t * fd,
	dbl_lpinfo * lp,
	dbl_ILLlp_cache * C,
	double * x,
	int nonZerosOnly)
{
	...
	for (j = 0; j < qslp->nstruct; j++)
	{
		...
		if (!nonZerosOnly || dbl_EGlpNumIsNeqZero (dx[j], PRINT_TOL))
		{
