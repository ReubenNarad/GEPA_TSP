	...
	mpf_factor_work * f)
{
	...
	mpf_EGlpNumCopy (f->fzero_tol, mpf_SZERO_TOLER);	/* 2^-50 */
	mpf_EGlpNumCopy (f->szero_tol, mpf_SZERO_TOLER);	/* 2^-50 */
	mpf_EGlpNumCopy (f->partial_tol, mpf_OBJBND_TOLER);	/* 2^-7 */
	f->ur_space_mul = 2.0;
	f->uc_space_mul = 1.1;
	f->lc_space_mul = 1.1;
	...
	f->dense_fract = 0.25;
	f->dense_min = 25;
	mpf_EGlpNumCopy (f->partial_cur, f->partial_tol);
	f->work_coef = 0;
	f->work_indx = 0;
	f->uc_inf = 0;
	f->ur_inf = 0;
	...
	f->crank = 0;
	f->dmat = 0;
	mpf_ILLsvector_init (&f->xtmp);
}

void mpf_ILLfactor_free_factor_work (
	mpf_factor_work * f)
{
#ifdef UPDATE_STUDY
	if (nupdate)
...
#endif
	mpf_EGlpNumFreeArray (f->work_coef);
	ILL_IFFREE (f->work_indx, int);

	ILL_IFFREE (f->uc_inf, mpf_uc_info);
	if (f->dim + f->max_k > 0 && f->ur_inf)
	{
		unsigned int i = f->dim + f->max_k + 1;

		while (i--)
			mpf_EGlpNumClearVar (f->ur_inf[i].max);
	}
	ILL_IFFREE (f->ur_inf, mpf_ur_info);
	ILL_IFFREE (f->lc_inf, mpf_lc_info);
	ILL_IFFREE (f->lr_inf, mpf_lr_info);
	ILL_IFFREE (f->er_inf, mpf_er_info);
	ILL_IFFREE (f->ucindx, int);
	ILL_IFFREE (f->ucrind, int);

	mpf_EGlpNumFreeArray (f->uccoef);
	ILL_IFFREE (f->urindx, int);
	ILL_IFFREE (f->urcind, int);

	mpf_EGlpNumFreeArray (f->urcoef);
	ILL_IFFREE (f->lcindx, int);

	mpf_EGlpNumFreeArray (f->lccoef);
	ILL_IFFREE (f->lrindx, int);

	mpf_EGlpNumFreeArray (f->lrcoef);
	ILL_IFFREE (f->erindx, int);

	mpf_EGlpNumFreeArray (f->ercoef);
	ILL_IFFREE (f->rperm, int);
	ILL_IFFREE (f->rrank, int);
	ILL_IFFREE (f->cperm, int);
	ILL_IFFREE (f->crank, int);

	mpf_EGlpNumFreeArray (f->dmat);
	mpf_ILLsvector_free (&f->xtmp);
}

int mpf_ILLfactor_set_factor_iparam (
	mpf_factor_work * f,
	int param,
	int val)
{
	...
	case QS_FACTOR_DENSE_MIN:
		...
		break;
	default:
		QSlog("Invalid param %d in mpf_ILLfactor_set_factor_iparam",
								param);
		return 1;
	}
	return 0;
...
int mpf_ILLfactor_set_factor_dparam (
	mpf_factor_work * f,
	int param,
	mpf_t val)
{
	...
	case QS_FACTOR_FZERO_TOL:
		mpf_EGlpNumCopy (f->fzero_tol, val);
		break;
	case QS_FACTOR_SZERO_TOL:
		mpf_EGlpNumCopy (f->szero_tol, val);
		break;
	case QS_FACTOR_UR_SPACE_MUL:
		f->ur_space_mul = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_UC_SPACE_MUL:
		f->uc_space_mul = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_LC_SPACE_MUL:
		f->lc_space_mul = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_LR_SPACE_MUL:
		f->lr_space_mul = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_ER_SPACE_MUL:
		f->er_space_mul = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_GROW_MUL:
		f->grow_mul = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_MAXMULT:
		f->maxmult = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_UPDMAXMULT:
		f->updmaxmult = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_DENSE_FRACT:
		f->dense_fract = mpf_EGlpNumToLf (val);
		break;
	case QS_FACTOR_PARTIAL_TOL:
		mpf_EGlpNumCopy (f->partial_tol, val);
		mpf_EGlpNumCopy (f->partial_cur, val);
		break;
	default:
		QSlog("Invalid param %d in mpf_ILLfactor_set_factor_dparam",
								param);
		return 1;
	}
	return 0;
	...
	int dim)
{
	...
	f->dim = dim;
	f->etacnt = 0;
	f->work_coef = mpf_EGlpNumAllocArray (dim);
	ILL_SAFE_MALLOC (f->work_indx, dim, int);

	ILL_SAFE_MALLOC (f->uc_inf, dim + (f->max_k + 1), mpf_uc_info);
	ILL_SAFE_MALLOC (f->ur_inf, dim + (f->max_k + 1), mpf_ur_info);
	ILL_SAFE_MALLOC (f->lc_inf, dim, mpf_lc_info);
	ILL_SAFE_MALLOC (f->lr_inf, dim, mpf_lr_info);
	...
	for (i = dim + f->max_k + 1; i--;)
		mpf_EGlpNumInitVar (f->ur_inf[i].max);

	for (i = 0; i < dim; i++)
	{
		mpf_EGlpNumZero (f->work_coef[i]);
		f->work_indx[i] = 0;
		f->uc_inf[i].nzcnt = 0;
		f->ur_inf[i].nzcnt = 0;
		f->lc_inf[i].nzcnt = 0;
	...
	rval = mpf_ILLsvector_alloc (&f->xtmp, dim);
...
CLEANUP:
	if (rval)
	{
		mpf_ILLfactor_free_factor_work (f);
	}
	EG_RETURN (rval);
}
#ifdef FACTOR_DEBUG
...
{
	int dim = f->dim;
	mpf_ur_info *ur_inf = f->ur_inf;
	mpf_uc_info *uc_inf = f->uc_inf;
	mpf_lc_info *lc_inf = f->lc_inf;
	mpf_lr_info *lr_inf = f->lr_inf;
	mpf_er_info *er_inf = f->er_inf;
	int nzcnt;
	int beg;
	...
	for (i = 0; i < dim; i++)
	{
		if (!remaining || ur_inf[i].next >= 0)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				...
				QSlog(" %.3f*%d", mpf_EGlpNumToLf (f->urcoef[beg + j]),
										f->urindx[beg + j]);
				if (f->urcind)
	...
	if (f->dmat)
	{
		...
		for (i = start; i < f->drows; i++)
		{
			QSlog("DRow %d %d (max %.3f):", i,
									...
									mpf_EGlpNumToLf (ur_inf[f->rperm[i + f->dense_base]].max));
			for (j = start; j < f->dcols; j++)
			{
				if (j == f->drows)
				{
				...
				QSlog(" %.3f", mpf_EGlpNumToLf (f->dmat[i * f->dcols + j]));
			}
		}
	}
	...
	if (!remaining)
	{
		for (i = 0; i < f->stage; i++)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				QSlog(" %.3f*%d", mpf_EGlpNumToLf (f->lccoef[beg + j]),
										f->lcindx[beg + j]);
			}
		}
		...
		for (i = 0; i < f->dim; i++)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				QSlog(" %.3f*%d", mpf_EGlpNumToLf (f->lrcoef[beg + j]),
										f->lrindx[beg + j]);
			}
		}
	}
	...
	if (!remaining)
	{
		for (i = 0; i < f->etacnt; i++)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				QSlog(" %.3f*%d", mpf_EGlpNumToLf (f->ercoef[beg + j]),
										f->erindx[beg + j]);
			}
		}
	}
	...
	for (i = 0; i < dim; i++)
	{
		if (!remaining || uc_inf[i].next >= 0)
		{
			...
			for (j = 0; j < nzcnt; j++)
			{
				if (f->uccoef != 0)
				{
					QSlog(" %.3f*%d", mpf_EGlpNumToLf (f->uccoef[beg + j]),
											f->ucindx[beg + j]);
					if (f->ucrind)
	...
	mpf_t * coef)
{
	...
	mpf_EGlpNumInitVar (ctmp);

	for (i = 1; i < nzcnt; i++)
	{
		itmp = indx[i];
		mpf_EGlpNumCopy (ctmp, coef[i]);
		for (j = i; j >= 1 && indx[j - 1] > itmp; j--)
		{
			indx[j] = indx[j - 1];
			mpf_EGlpNumCopy (coef[j], coef[j - 1]);
		}
		indx[j] = itmp;
		mpf_EGlpNumCopy (coef[j], ctmp);
	}
	mpf_EGlpNumClearVar (ctmp);
}
	...
	mpf_factor_work * f)
{
	...
	for (r = 0; r < f->dim; r++)
	{
		...
		for (j = 0; j < nzcnt; j++)
		{
			...
			if (fabs(mpf_EGlpNumToLf(f->uccoef[cbeg + f->urcind[rbeg + j]]) - mpf_EGlpNumToLf(f->urcoef[rbeg + j]))>1000*mpf_EGlpNumToLf(mpf_epsLpNum))
			{
	...
	mpf_factor_work * f)
{
	...
	mpf_lc_info *lc_inf = f->lc_inf;
	mpf_er_info *er_inf = f->er_inf;
	mpf_t *urcoef = f->urcoef;
	mpf_t *lccoef = f->lccoef;
	mpf_t *ercoef = f->ercoef;
	int lnzcnt = 0;
	int unzcnt = 0;
	int enzcnt = 0;
	int nzcnt;
	...
	mpf_EGlpNumInitVar (umax);
	mpf_EGlpNumInitVar (lmax);
	mpf_EGlpNumInitVar (emax);
	mpf_EGlpNumZero (umax);
	for (i = 0; i < dim; i++)
	{
		nzcnt = ur_inf[i].nzcnt;
		beg = ur_inf[i].rbeg;
		...
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumSetToMaxAbs (umax, urcoef[beg + j]);
		}
	}
	mpf_EGlpNumZero (lmax);
	for (i = 0; i < dim; i++)
	{
		nzcnt = lc_inf[i].nzcnt;
		beg = lc_inf[i].cbeg;
		...
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumSetToMaxAbs (lmax, lccoef[beg + j]);
		}
	}
	mpf_EGlpNumZero (emax);
	for (i = 0; i < ecnt; i++)
	{
		nzcnt = er_inf[i].nzcnt;
		beg = er_inf[i].rbeg;
		...
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumSetToMaxAbs (emax, ercoef[beg + j]);
		}
	}
	MESSAGE(0, "factor U %d nzs %.3e max L %d nzs %.3e max E %d nzs %.3e max",
					unzcnt, mpf_EGlpNumToLf (umax), lnzcnt, mpf_EGlpNumToLf (lmax), enzcnt,
					mpf_EGlpNumToLf (emax));
	mpf_EGlpNumClearVar (umax);
	mpf_EGlpNumClearVar (lmax);
	mpf_EGlpNumClearVar (emax);
}
#endif
	...
	mpf_factor_work * f)
{
	int i;
	int dim = f->dim;
	mpf_t *work_coef = f->work_coef;

	for (i = 0; i < dim; i++)
	{
		mpf_EGlpNumZero (work_coef[i]);
	}
}
...
static void load_row (
	...
	int r)
{
	mpf_t *prow_urcoef = f->urcoef + f->ur_inf[r].rbeg;
	...
	{
		j = prow_urindx[i];
		mpf_EGlpNumCopy (work_coef[j], prow_urcoef[i]);
		work_indx[j] = 1;
	}
}
...
#endif
	new_urcoef = mpf_EGlpNumAllocArray (minspace);
	...
	if (urcind)
	{
		for (j = 0; j < dim; j++)
		{
			...
			for (i = 0; i < nzcnt; i++)
			{
				...
				mpf_EGlpNumCopy (new_urcoef[new_nzcnt], urcoef[rbeg + i]);
	...
	int space)
{
	...
	if (ucrind)
	{
		new_uccoef = mpf_EGlpNumAllocArray (minspace);
	...
	if (ucrind)
	{
		for (j = 0; j < dim; j++)
		{
			...
			for (i = 0; i < nzcnt; i++)
			{
				...
				mpf_EGlpNumCopy (new_uccoef[new_nzcnt], uccoef[cbeg + i]);
	...
	int space)
{
	...
	new_lccoef = mpf_EGlpNumAllocArray (minspace);
	ILL_SAFE_MALLOC (new_lcindx, minspace, int);

	for (i = 0; i < lc_freebeg; i++)
	{
		mpf_EGlpNumCopy (new_lccoef[i], lccoef[i]);
		new_lcindx[i] = lcindx[i];
	}

	mpf_EGlpNumFreeArray (lccoef);
	f->lccoef = new_lccoef;
	new_lccoef = 0;
	...
	int c)
{
	...
	mpf_t *urcoef = f->urcoef + ur_inf[r].rbeg;
	...
	mpf_EGlpNumInitVar (tcoef);
	mpf_EGlpNumInitVar (max);
	mpf_EGlpNumZero (max);
	...
	for (i = 0; i < pivcnt; i++)
	{
		if (urindx[i] == c)
		{
			...
			mpf_EGLPNUM_SWAP (urcoef[i], urcoef[pivcnt], tcoef);
			--i;
		}
		else
		{
			mpf_EGlpNumSetToMaxAbs (max, urcoef[i]);
		}
	}
	ur_inf[r].pivcnt = pivcnt;
	mpf_EGlpNumCopy (ur_inf[r].max, max);
	set_row_nz (f, r);
	mpf_EGlpNumClearVar (max);
	mpf_EGlpNumClearVar (tcoef);
}
	...
	mpf_t * coef)
{
	...
	mpf_EGlpNumZero (*coef);
	for (i = 0; i < prow_nzcnt; i++)
	{
		if (prow_urindx[i] == c)
		{
			mpf_EGlpNumCopy (*coef, prow_urcoef[i]);
			return;
		}
	}
	...
	mpf_t * p_pivot_coef)
{
	...
	mpf_EGlpNumInitVar (elim_coef);
	mpf_EGlpNumZero (max);
	find_coef (f, r, c, &elim_coef);
	mpf_EGlpNumDivTo (elim_coef, work_coef[c]);
	mpf_EGlpNumCopy (*p_pivot_coef, elim_coef);
	...
	for (i = 0; i < prow_nzcnt; i++)
	{
		...
		if (work_indx[j] == 1)
		{
			mpf_EGlpNumCopy (x, urcoef[prow_beg + i]);
			mpf_EGlpNumSubInnProdTo (x, elim_coef, work_coef[j]);
			if ((!(mpf_EGlpNumIsNeqZero (x, f->fzero_tol))) || j == c)
			{
				cancel++;
				if (j != c)
				{
				...
				if (i < prow_pivcnt)
				{
					...
					mpf_EGlpNumCopy (urcoef[prow_beg + i], urcoef[prow_beg + prow_pivcnt]);
					if (prow_pivcnt != prow_nzcnt)
					{
						urindx[prow_beg + prow_pivcnt] = urindx[prow_beg + prow_nzcnt];
						mpf_EGlpNumCopy (urcoef[prow_beg + prow_pivcnt],
				...
				else
				{
					...
					mpf_EGlpNumCopy (urcoef[prow_beg + i], urcoef[prow_beg + prow_nzcnt]);
				}
			...
			{
				mpf_EGlpNumCopy (urcoef[prow_beg + i], x);
				if (i < prow_pivcnt)
				{
					mpf_EGlpNumSetToMaxAbs (max, x);
				}
			}
			work_indx[j] = 0;
			fill--;
		...
		else
		{
			if (i < prow_pivcnt)
			{
				mpf_EGlpNumSetToMaxAbs (max, urcoef[prow_beg + i]);
			}
		}
	}
	...
	if (fill > 0)
	{
		...
		if (fill > cancel)
		{
			...
			for (i = 0; i < prow_nzcnt; i++)
			{
				...
				mpf_EGlpNumCopy (urcoef[ur_freebeg + i], urcoef[prow_beg + i]);
				urindx[prow_beg + i] = -1;
			}
		...
		for (i = 0; i < erow_pivcnt; i++)
		{
			...
			if (work_indx[j] == 1)
			{
				mpf_EGlpNumCopyNeg (x, elim_coef);
				mpf_EGlpNumMultTo (x, urcoef[erow_beg + i]);
				if (mpf_EGlpNumIsNeqZero (x, f->fzero_tol))
				{
					...
					if (prow_pivcnt != prow_nzcnt)
					{
						...
						mpf_EGlpNumCopy (urcoef[prow_beg + prow_nzcnt],
												 urcoef[prow_beg + prow_pivcnt]);
					}
					urindx[prow_beg + prow_pivcnt] = j;
					mpf_EGlpNumCopy (urcoef[prow_beg + prow_pivcnt], x);
					mpf_EGlpNumSetToMaxAbs (max, x);
					prow_pivcnt++;
					prow_nzcnt++;
				}
		...
		for (i = erow_pivcnt; i < erow_nzcnt; i++)
		{
			...
			if (work_indx[j] == 1)
			{
				...
				if (mpf_EGlpNumIsNeqZero (x, f->fzero_tol))
				{
					...
					mpf_EGlpNumCopy (urcoef[prow_beg + prow_nzcnt], x);
					prow_nzcnt++;
				}
			}
			else
	...
	if (uc_inf[c].nzcnt == 1)
	{
		...
		for (i = 0; i < nzcnt; i++)
		{
			...
			if (j == c)
			{
				urindx[i] = urindx[0];
				urindx[0] = c;
				mpf_EGLPNUM_SWAP (urcoef[0], urcoef[i], pivot_coef);
			}
		}
		remove_row (f, r);
		remove_col (f, c);
	...
	else
	{
		...
		for (i = 0; i < nzcnt; i++)
		{
			...
			if (j != r)
			{
				...
				CHECKRVALG (rval, CLEANUP);
				lcindx[lc_freebeg] = j;
				mpf_EGlpNumCopy (lccoef[lc_freebeg], pivot_coef);
				lc_freebeg++;
#ifdef TRACK_FACTOR
				mpf_EGlpNumSetToMaxAbs (f->maxelem_factor, pivot_coef);
				if (mpf_EGlpNumIsLess (f->maxelem_factor, ur_inf[r].max))
					mpf_EGlpNumCopy (f->maxelem_factor, ur_inf[r].max);
#endif /* TRACK_FACTOR */
			}
		}
...
CLEANUP:
	mpf_EGlpNumClearVar (pivot_coef);
	EG_RETURN (rval);
}
	...
	int *p_r)
{
	...
	mpf_EGlpNumInitVar (num_tmp[0]);
	mpf_EGlpNumInitVar (num_tmp[1]);
	...
	*p_r = -1;
	for (i = 0; i < nzcnt; i++)
	{
		...
		if((bestnz == -1 || ur_inf[r].pivcnt < bestnz))
		{
			...
			if(mpf_EGlpNumIsLessZero(num_tmp[0]))
				mpf_EGlpNumSign (num_tmp[0]);
			mpf_EGlpNumCopy (num_tmp[1], f->partial_cur);
			mpf_EGlpNumMultTo (num_tmp[1], ur_inf[r].max);
			if(mpf_EGlpNumIsLeq (num_tmp[1], num_tmp[0]))
			{
	...
	mpf_EGlpNumClearVar (num_tmp[0]);
	mpf_EGlpNumClearVar (num_tmp[1]);
}
	...
	int *p_c)
{
	...
	mpf_EGlpNumInitVar (thresh[0]);
	mpf_EGlpNumInitVar (thresh[1]);
	mpf_EGlpNumCopy (thresh[0], f->partial_cur);
	mpf_EGlpNumMultTo (thresh[0], ur_inf[r].max);
	*p_c = -1;
	for (i = 0; i < pivcnt; i++)
	{
		c = urindx[rbeg + i];
		...
		{
			mpf_EGlpNumCopyAbs (thresh[1], urcoef[rbeg + i]);
			if(mpf_EGlpNumIsLeq (thresh[0], thresh[1]))
			{
	...
	mpf_EGlpNumClearVar (thresh[0]);
	mpf_EGlpNumClearVar (thresh[1]);
}
	...
	mpf_factor_work * f)
{
	...
	if (f->urindx == 0 || f->urcoef == 0)
	{
		...
		f->urcoef = mpf_EGlpNumAllocArray (f->ur_space);
	}
	...
	if (f->lcindx == 0 || f->lccoef == 0)
	{
		...
		f->lccoef = mpf_EGlpNumAllocArray (f->lc_space);
	}
	...
	mpf_t * in_uccoef)
{
	...
	for (i = 0; i < dim; i++)
	{
		...
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumCopy (v, in_uccoef[beg + j]);
			if (!(mpf_EGlpNumIsNeqZero (v, f->fzero_tol)))
				continue;
			r = in_ucindx[beg + j];
			ucindx[uc_inf[i].nzcnt++] = r;
			urindx[ur_inf[r].nzcnt] = i;
			mpf_EGlpNumCopy (urcoef[ur_inf[r].nzcnt], v);
			ur_inf[r].nzcnt++;
		}
	}
	...
	for (i = 0; i < dim; i++)
	{
		...
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumSetToMaxAbs (max, urcoef[beg + j]);
		}
		mpf_EGlpNumCopy (ur_inf[i].max, max);
	}
	...
	for (i = 0; i < dim; i++)
	{
		if (mpf_EGlpNumIsLess (max, ur_inf[i].max))
			mpf_EGlpNumCopy (max, ur_inf[i].max);
		nzcnt += ur_inf[i].nzcnt;
	}

	mpf_EGlpNumCopy (f->maxelem_orig, max);
	f->nzcnt_orig = nzcnt;
	mpf_EGlpNumCopy (f->maxelem_factor, f->maxelem_orig);
	...
	mpf_factor_work * f)
{
	...
	mpf_EGlpNumFreeArray (f->uccoef);
	uccoef = mpf_EGlpNumAllocArray (nzcnt);
	f->uccoef = uccoef;
	...
	for (i = 0; i < dim; i++)
	{
		...
		if (cnzcnt != 0)
		{
			...
			mpf_EGlpNumCopy (uccoef[cbeg + cnzcnt], uccoef[cbeg]);
			ucrind[cbeg + cnzcnt] = ucrind[cbeg];
			urcind[ur_inf[ucindx[cbeg]].rbeg + ucrind[cbeg]] = cnzcnt;
		}
		ucindx[cbeg] = i;
		mpf_EGlpNumCopy (uccoef[cbeg], urcoef[beg]);
		ucrind[cbeg] = 0;
		urcind[beg] = 0;
		...
		for (j = 1; j < nzcnt; j++)
		{
			...
			mpf_EGlpNumCopy (uccoef[cbeg + cnzcnt], urcoef[beg + j]);
	...
	ILL_SAFE_MALLOC (f->er_inf, f->etamax, mpf_er_info);
	ILL_SAFE_MALLOC (f->erindx, er_space, int);

	f->ercoef = mpf_EGlpNumAllocArray (er_space);
	f->etacnt = 0;
	f->er_freebeg = 0;
	f->er_space = er_space;
	...
	mpf_factor_work * f)
{
	...
	if (nzcnt)
	{
		lrcoef = mpf_EGlpNumAllocArray (nzcnt);
		f->lrcoef = lrcoef;
	}
	...
	for (i = 0; i < dim; i++)
	{
		...
		for (j = 0; j < nzcnt; j++)
		{
			...
			mpf_EGlpNumCopy (lrcoef[rbeg + rnzcnt], lccoef[beg + j]);
			lr_inf[k].nzcnt++;
		}
	}
	...
	mpf_EGlpNumCopy (f->maxelem_cur, f->maxelem_factor);
	...
	mpf_factor_work * f)
{
	...
	dmat = mpf_EGlpNumAllocArray (dsize);

	for (i = 0; i < dsize; i++)
		mpf_EGlpNumZero (dmat[i]);
	...
	for (i = 0; i < drows; i++)
	{
		...
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumCopy (dmat[i * dcols - stage + crank[urindx[beg + j]]],
									 urcoef[beg + j]);
		}
	}
	...
	int *p_c)
{
	...
	mpf_EGlpNumInitVar (maxval);
	mpf_EGlpNumZero (maxval);
	max_r = -1;
	for (i = s; i < drows; i++)
	{
		if (mpf_EGlpNumIsLess (maxval, ur_inf[rperm[dense_base + i]].max))
		{
			mpf_EGlpNumCopy (maxval, ur_inf[rperm[dense_base + i]].max);
			max_r = i;
		}
	}
	if (max_r == -1)
	...
	int c)
{
	...
	if (r != s)
	{
		...
		for (i = 0; i < dcols; i++)
		{
			mpf_EGLPNUM_SWAP (dmat[s * dcols + i], dmat[r * dcols + i], v);
		}
	}
	if (c != s)
	{
		...
		for (i = 0; i < drows; i++)
		{
			mpf_EGLPNUM_SWAP (dmat[i * dcols + s], dmat[i * dcols + c], v);
		}
	}
	mpf_EGlpNumClearVar (v);
}
	...
	int c)
{
	...
	mpf_EGlpNumInitVar (maxelem_factor);
	mpf_EGlpNumCopy (maxelem_factor, f->maxelem_factor);
#endif
	mpf_EGlpNumInitVar (pivval);
	mpf_EGlpNumInitVar (max);
	mpf_EGlpNumInitVar (v);
	mpf_EGlpNumInitVar (w);

	dense_swap (f, r, c);
	f->stage++;
	mpf_EGlpNumCopyFrac (pivval, mpf_oneLpNum, dmat[s * dcols + s]);
	for (i = s + 1; i < drows; i++)
	{
		mpf_EGlpNumCopy (v, dmat[i * dcols + s]);
		if (mpf_EGlpNumIsNeqqZero (v))
		{
			mpf_EGlpNumMultTo (v, pivval);
			if (mpf_EGlpNumIsNeqZero (v, f->fzero_tol))
			{
				mpf_EGlpNumCopy (dmat[i * dcols + s], v);
#ifdef TRACK_FACTOR
				mpf_EGlpNumSetToMaxAbs (maxelem_factor, v);
#endif
				mpf_EGlpNumZero (max);
				for (j = s + 1; j < drows; j++)
				{
					mpf_EGlpNumCopy (w, dmat[i * dcols + j]);
					mpf_EGlpNumSubInnProdTo (w, v, dmat[s * dcols + j]);
					mpf_EGlpNumCopy (dmat[i * dcols + j], w);
					mpf_EGlpNumSetToMaxAbs (max, w);
				}
				...
				mpf_EGlpNumCopy (ur_inf[rperm[dense_base + i]].max, max);
#ifdef TRACK_FACTOR
				if (mpf_EGlpNumIsLess (maxelem_factor, max))
					mpf_EGlpNumCopy (maxelem_factor, max);
#endif
			}
			else
			{
				mpf_EGlpNumZero (dmat[i * dcols + s]);
			}
		}
	}
#ifdef TRACK_FACTOR
	mpf_EGlpNumCopy (f->maxelem_factor, maxelem_factor);
	mpf_EGlpNumClearVar (maxelem_factor);
#endif
	mpf_EGlpNumClearVar (pivval);
	mpf_EGlpNumClearVar (max);
	mpf_EGlpNumClearVar (v);
	mpf_EGlpNumClearVar (w);
}
	...
	int i)
{
	...
	for (j = i; j < dcols; j++)
	{
		if (mpf_EGlpNumIsNeqZero (dmat[j], f->fzero_tol))
		{
			nzcnt++;
		}
	}
	...
	for (j = i; j < dcols; j++)
	{
		if (mpf_EGlpNumIsNeqZero (dmat[j], f->fzero_tol))
		{
			mpf_EGlpNumCopy (urcoef[beg], dmat[j]);
	...
	int i)
{
	...
	for (j = i + 1; j < drows; j++)
	{
		if (mpf_EGlpNumIsNeqZero (dmat[j * dcols + i], f->fzero_tol))
		{
			nzcnt++;
		}
	}
	...
	for (j = i + 1; j < drows; j++)
	{
		if (mpf_EGlpNumIsNeqZero (dmat[j * dcols + i], f->fzero_tol))
		{
			mpf_EGlpNumCopy (lccoef[beg], dmat[j * dcols + i]);
...
#ifdef FACTOR_DEBUG
#if (FACTOR_DEBUG+0>1)
	MESSAGE (0,"before Dense mpf_ILLfactor");
	dump_matrix (f, 1);
#endif
#endif
...
#ifdef TRACK_FACTOR
#ifdef NOTICE_BLOWUP
		tmpsize = f->maxmult * mpf_EGlpNumToLf (f->maxelem_orig);
		if (tmpsize < mpf_EGlpNumToLf (f->maxelem_factor) &&
				mpf_EGlpNumIsLess (f->partial_cur, mpf_oneLpNum))
		{
...
#ifdef FACTOR_DEBUG
	MESSAGE (0,"After dense mpf_ILLfactor:\n");
	dump_matrix (f, 0);
#endif /* FACTOR_DEBUG */
		...
		for (i = 0; i < ncol; i++)
		{
			...
			for (j = 0; j < clen[i]; j++)
			{
				EGioPrintf (fsave, " %d %.16lg", cindx[cbeg[i] + j],
								 mpf_EGlpNumToLf (ccoef[cbeg[i] + j]));
			}
			EGioPrintf (fsave, "\n");
		}
		EGioPrintf (fsave, "\n");
...
#ifdef TRACK_FACTOR
#ifdef NOTICE_BLOWUP
		mpf_EGlpNumSet (tmpsize, f->maxmult);
		mpf_EGlpNumMultTo (tmpsize, f->maxelem_orig);
		if (mpf_EGlpNumIsLess (tmpsize, f->maxelem_factor) &&
				mpf_EGlpNumIsLess (f->partial_cur, mpf_oneLpNum))
		{
			return E_FACTOR_BLOWUP;
		}
...
#ifdef TRACK_FACTOR
#ifdef NOTICE_BLOWUP
	mpf_EGlpNumSet (tmpsize, f->minmult);
	mpf_EGlpNumMultTo (tmpsize, f->maxelem_orig);
	if (mpf_EGlpNumIsLess (f->maxelem_factor, tmpsize) &&
			mpf_EGlpNumIsLess (f->partial_tol, f->partial_cur))
	{
		if (mpf_EGlpNumIsGreaDbl (f->partial_cur, 0.5))
		{
			mpf_EGlpNumSet (f->partial_cur, 0.5);
		}
		else if (mpf_EGlpNumIsGreaDbl (f->partial_cur, 0.25))
		{
			mpf_EGlpNumSet (f->partial_cur, 0.25);
		}
		else if (mpf_EGlpNumIsGreaDbl (f->partial_cur, 0.1))
		{
			mpf_EGlpNumSet (f->partial_cur, 0.1);
		}
		else
		{
			mpf_EGlpNumDivUiTo (f->partial_cur, 10);
		}
		if (mpf_EGlpNumIsLess (f->partial_cur, f->partial_tol))
		{
			mpf_EGlpNumCopy (f->partial_cur, f->partial_tol);
		}
...
AGAIN:
	...
	{
		if (mpf_EGlpNumIsLessDbl (f->partial_cur, 0.1))
		{
			mpf_EGlpNumMultUiTo (f->partial_cur, 10);
		}
		else if (mpf_EGlpNumIsLessDbl (f->partial_cur, 0.25))
		{
			mpf_EGlpNumSet (f->partial_cur, 0.25);
		}
		else if (mpf_EGlpNumIsLessDbl (f->partial_cur, 0.5))
		{
			mpf_EGlpNumSet (f->partial_cur, 0.5);
		}
		else if (mpf_EGlpNumIsLess (f->partial_cur, mpf_oneLpNum))
		{
			mpf_EGlpNumOne (f->partial_cur);
		}
		else
		{
			EG_RETURN (rval);
	...
	mpf_t * a)
{
	...
	for (i = 0; i < dim; i++)
	{
		mpf_EGlpNumCopy (v, a[lc_inf[i].c]);
		if (mpf_EGlpNumIsNeqqZero (v))
		{
			nzcnt = lc_inf[i].nzcnt;
			beg = lc_inf[i].cbeg;
			for (j = 0; j < nzcnt; j++)
			{
				mpf_EGlpNumSubInnProdTo (a[lcindx[beg + j]], v, lccoef[beg + j]);
			}
		}
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpf_ILLfactor_ftran a after l %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpf_EGlpNumToLf (a[j]));
		}
		MESSAGE(0," ");
#endif
#endif /* SOLVE_DEBUG */
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpf_ILLfactor_ftran a after l:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpf_EGlpNumToLf (a[j]));
	}
	...
	mpf_svector * x)
{
	...
	mpf_EGlpNumCopy (v, work[c]);
	mpf_EGlpNumZero (work[c]);
	if (mpf_EGlpNumIsNeqqZero (v))
	{
		x->indx[x->nzcnt] = c;
		mpf_EGlpNumCopy (x->coef[x->nzcnt], v);
		x->nzcnt++;
	}
	c = lc_inf[c].crank;
	nzcnt = lc_inf[c].nzcnt;
	...
	{
		c = indx[i];
		mpf_EGlpNumSubInnProdTo (work[c], v, coef[i]);
		if (--lc_inf[c].delay == 0)
		{
	...
	mpf_svector * x)
{
	...
	for (i = 0; i < anzcnt; i++)
	{
		...
		mpf_EGlpNumCopy (work[aindx[i]], acoef[i]);
	}
	x->nzcnt = 0;
	for (i = 0; i < anzcnt; i++)
	{
...
#ifdef SOLVE_DEBUG
	QSlog("mpf_ILLfactor_ftran x after l3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
#endif /* SOLVE_DEBUG */
}
	...
	mpf_t * a)
{
	...
	for (i = 0; i < etacnt; i++)
	{
		mpf_EGlpNumCopy (v, a[er_inf[i].r]);
		nzcnt = er_inf[i].nzcnt;
		beg = er_inf[i].rbeg;
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumSubInnProdTo (v, ercoef[beg + j], a[erindx[beg + j]]);
		}
		mpf_EGlpNumCopy (a[er_inf[i].r], v);
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpf_ILLfactor_ftran a after eta %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpf_EGlpNumToLf (a[j]));
		}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpf_ILLfactor_ftran a after eta:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpf_EGlpNumToLf (a[j]));
	}
	...
	mpf_svector * a)
{
	...
	for (i = 0; i < anzcnt; i++)
	{
		mpf_EGlpNumCopy (work_coef[aindx[i]], acoef[i]);
		work_indx[aindx[i]] = i + 1;
	}
	for (i = 0; i < etacnt; i++)
	{
		r = er_inf[i].r;
		mpf_EGlpNumCopy (v, work_coef[r]);
		nzcnt = er_inf[i].nzcnt;
		beg = er_inf[i].rbeg;
		for (j = 0; j < nzcnt; j++)
		{
			mpf_EGlpNumSubInnProdTo (v, ercoef[beg + j], work_coef[erindx[beg + j]]);
		}
		if (mpf_EGlpNumIsNeqqZero (v))
		{
			mpf_EGlpNumCopy (work_coef[r], v);
			if (work_indx[r] == 0)
			{
				mpf_EGlpNumCopy (acoef[anzcnt], v);
				aindx[anzcnt] = r;
			...
			{
				mpf_EGlpNumCopy (acoef[work_indx[r] - 1], v);
			}
		}
		else
		{
			mpf_EGlpNumZero (work_coef[r]);
			if (work_indx[r])
			{
				mpf_EGlpNumZero (acoef[work_indx[r] - 1]);
			}
		}
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpf_ILLfactor_ftran a after eta2 %d:", i);
		for (j = 0; j < anzcnt; j++)
		{
			QSlog(" %.3f*%d", mpf_EGlpNumToLf (acoef[j]), aindx[j]);
		}
#endif
#endif /* SOLVE_DEBUG */
	}
	...
	while (i < anzcnt)
	{
		mpf_EGlpNumZero (work_coef[aindx[i]]);
		work_indx[aindx[i]] = 0;
		if (mpf_EGlpNumIsNeqZero (acoef[i], f->fzero_tol))
		{
		...
		{
			--anzcnt;
			mpf_EGlpNumCopy (acoef[i], acoef[anzcnt]);
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpf_ILLfactor_ftran a after eta2:");
	for (j = 0; j < anzcnt; j++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (acoef[j]), aindx[j]);
	}
	...
	mpf_svector * x)
{
	...
	for (i = dim - 1; i >= 0; i--)
	{
		mpf_EGlpNumCopy (v, a[rperm[i]]);
		if (mpf_EGlpNumIsNeqqZero (v))	/*((v = a[rperm[i]]) != 0.0) */
		{
			j = cperm[i];
			beg = uc_inf[j].cbeg;
			mpf_EGlpNumDivTo (v, uccoef[beg]);
			if (mpf_EGlpNumIsNeqZero (v, f->szero_tol))
			{
				/*if (v > szero_tol || v < -szero_tol) */
				xindx[xnzcnt] = j;
				mpf_EGlpNumCopy (xcoef[xnzcnt], v);
				xnzcnt++;
			}
			nzcnt = uc_inf[j].nzcnt;
			for (j = 1; j < nzcnt; j++)
			{
				mpf_EGlpNumSubInnProdTo (a[ucindx[beg + j]], v, uccoef[beg + j]);
			}
			mpf_EGlpNumZero (a[rperm[i]]);
		}
	}
	x->nzcnt = xnzcnt;
#ifdef SOLVE_DEBUG
	QSlog("mpf_ILLfactor_ftran x after u:");
	for (j = 0; j < x->nzcnt; j++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (x->coef[j]), x->indx[j]);
	}
#endif /* SOLVE_DEBUG */
	mpf_EGlpNumClearVar (v);
}
...
#ifdef SOLVE_DEBUG
	QSlog("mpf_ILLfactor_ftran x after u3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
...
/* mpf_ILLfactor_ftran solves Bx=a for x */
void mpf_ILLfactor_ftran (
	mpf_factor_work * f,
	mpf_svector * a,
	mpf_svector * x)
{
...
#ifdef RECORD
	{
		...
		for (i = 0; i < a->nzcnt; i++)
		{
			EGioPrintf (fsave, " %d %.16e", a->indx[i], mpf_EGlpNumToLf (a->coef[i]));
		}
		EGioPrintf (fsave, "\n");
		EGioFlush (fsave);
	}
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpf_ILLfactor_ftran a:");
		for (i = 0; i < a->nzcnt; i++)
		{
			QSlog(" %d %la", a->indx[i], mpf_EGlpNumToLf (a->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpf_ILLfactor_ftran x:");
		for (i = 0; i < x->nzcnt; i++)
		{
			QSlog(" %d %la", x->indx[i], mpf_EGlpNumToLf (x->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
	return;
...
/* mpf_ILLfactor_ftran_update solves Bx=a for x, and also returns upd, where Ux=upd */
void mpf_ILLfactor_ftran_update (
	mpf_factor_work * f,
	mpf_svector * a,
	mpf_svector * upd,
	mpf_svector * x)
{
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpf_ILLfactor_ftran_update a:");
		for (i = 0; i < a->nzcnt; i++)
		{
			QSlog(" %d %.3f", a->indx[i], mpf_EGlpNumToLf (a->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
	...
	else
	{
		...
		for (i = 0; i < dim; i++)
		{
			if (mpf_EGlpNumIsNeqqZero (work_coef[i]))
			{
				if (mpf_EGlpNumIsNeqZero (work_coef[i], f->szero_tol))
					/*if(work_coef[i] > szero_tol || work_coef[i] < -szero_tol) */
				{
					aindx[nzcnt] = i;
					mpf_EGlpNumCopy (acoef[nzcnt], work_coef[i]);
					nzcnt++;
				}
			}
		}
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpf_ILLfactor_ftran update x:");
		for (i = 0; i < x->nzcnt; i++)
		{
			QSlog(" %d %.3f", x->indx[i], mpf_EGlpNumToLf (x->coef[i]));
		}
	}
#endif /* DEBUG_FACTOR */
}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpf_ILLfactor_btran x before l2 %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpf_EGlpNumToLf (x[j]));
		}
#endif
#endif /* SOLVE_DEBUG */
		mpf_EGlpNumCopy (v, x[lr_inf[i].r]);
		if (mpf_EGlpNumIsNeqqZero (v))
		{
			nzcnt = lr_inf[i].nzcnt;
			beg = lr_inf[i].rbeg;
			for (j = 0; j < nzcnt; j++)
			{
				mpf_EGlpNumSubInnProdTo (x[lrindx[beg + j]], v, lrcoef[beg + j]);
			}
		}
	}
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpf_ILLfactor_btran x after l2:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpf_EGlpNumToLf (x[j]));
	}
	...
	mpf_svector * x)
{
	...
	mpf_EGlpNumCopy (v, work[r]);
	mpf_EGlpNumZero (work[r]);
	if (mpf_EGlpNumIsNeqZero (v, f->szero_tol))
		/*if (v > szero_tol || v < -szero_tol) */
	{
		x->indx[x->nzcnt] = r;
	...
	{
		r = indx[i];
		mpf_EGlpNumSubInnProdTo (work[r], v, coef[i]);
		if (--lr_inf[r].delay == 0)
		{
...
#ifdef SOLVE_DEBUG
	QSlog("mpf_ILLfactor_btran x after l3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpf_ILLfactor_btran x before eta %d:", i);
		for (j = 0; j < f->dim; j++)
		{
			QSlog(" %.3f", mpf_EGlpNumToLf (x[j]));
		}
#endif
#endif /* SOLVE_DEBUG */
		mpf_EGlpNumCopy (v, x[er_inf[i].r]);
		if (mpf_EGlpNumIsNeqqZero (v))
		{
			nzcnt = er_inf[i].nzcnt;
			beg = er_inf[i].rbeg;
			for (j = 0; j < nzcnt; j++)
			{
				mpf_EGlpNumSubInnProdTo (x[erindx[beg + j]], v, ercoef[beg + j]);
			}
		}
	}
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpf_ILLfactor_btran x after eta:");
	for (j = 0; j < f->dim; j++)
	{
		QSlog(" %.3f", mpf_EGlpNumToLf (x[j]));
	}
	...
	mpf_svector * x)
{
	...
	for (i = 0; i < xnzcnt; i++)
	{
		mpf_EGlpNumCopy (work_coef[xindx[i]], xcoef[i]);
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 > 1)
		QSlog("mpf_ILLfactor_btran x before eta2 %d:", i);
		for (j = 0; j < xnzcnt; j++)
		{
			QSlog(" %.3f*%d", mpf_EGlpNumToLf (work_coef[xindx[j]]), xindx[j]);
		}
#endif
#endif /* SOLVE_DEBUG */
		mpf_EGlpNumCopy (v, work_coef[er_inf[i].r]);
		if (mpf_EGlpNumIsNeqqZero (v))
		{
			nzcnt = er_inf[i].nzcnt;
			beg = er_inf[i].rbeg;
			for (j = 0; j < nzcnt; j++)
			{
				...
				mpf_EGlpNumSubInnProdTo (work_coef[erindx[beg + j]], v, ercoef[beg + j]);
			}
		}
	}
	...
	while (j < xnzcnt)
	{
		mpf_EGlpNumCopy (xcoef[j], work_coef[xindx[j]]);
		mpf_EGlpNumZero (work_coef[xindx[j]]);
		work_indx[xindx[j]] = 0;
		if (!mpf_EGlpNumIsNeqqZero (xcoef[j]))
		{
			--xnzcnt;
...
#ifdef SOLVE_DEBUG
#if (SOLVE_DEBUG+0 <= 1)
	QSlog("mpf_ILLfactor_btran x after eta2:");
	for (j = 0; j < xnzcnt; j++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (xcoef[j]), xindx[j]);
	}
#endif
#endif /* SOLVE_DEBUG */
	mpf_EGlpNumClearVar (v);
	...
	mpf_svector * x)
{
	...
	for (i = 0; i < dim; i++)
	{
		mpf_EGlpNumCopy (v, a[cperm[i]]);
		if (mpf_EGlpNumIsNeqqZero (v))
		{
			j = rperm[i];
			beg = ur_inf[j].rbeg;
			mpf_EGlpNumDivTo (v, urcoef[beg]);
			if (mpf_EGlpNumIsNeqZero (v, f->szero_tol))	/*
																							 * if (v > szero_tol || v < -szero_tol) */
			{
				xindx[xnzcnt] = j;
			...
			for (j = 1; j < nzcnt; j++)
			{
				mpf_EGlpNumSubInnProdTo (a[urindx[beg + j]], v, urcoef[beg + j]);
			}
			mpf_EGlpNumZero (a[cperm[i]]);
		}
	}
	x->nzcnt = xnzcnt;
#ifdef SOLVE_DEBUG
	QSlog("mpf_ILLfactor_btran x after u:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
...
#ifdef SOLVE_DEBUG
	QSlog("mpf_ILLfactor_btran x after u3:");
	for (i = 0; i < x->nzcnt; i++)
	{
		QSlog(" %.3f*%d", mpf_EGlpNumToLf (x->coef[i]), x->indx[i]);
	}
...
/* mpf_ILLfactor_btran solves x^tB=a^t (or, B^t x = a) for x */
void mpf_ILLfactor_btran (
	mpf_factor_work * f,
	mpf_svector * a,
	mpf_svector * x)
{
...
static int expand_col (
	...
	int col)
{
	mpf_uc_info *uc_inf = f->uc_inf + col;
	int uc_freebeg = f->uc_freebeg;
	...
	for (i = 0; i < nzcnt; i++)
	{
		mpf_EGlpNumCopy (uccoef[uc_freebeg + i], uccoef[cbeg + i]);
		ucindx[uc_freebeg + i] = ucindx[cbeg + i];
...
static int expand_row (
	...
	int row)
{
	mpf_ur_info *ur_inf = f->ur_inf + row;
	int ur_freebeg = f->ur_freebeg;
	...
	for (i = 0; i < nzcnt; i++)
	{
		mpf_EGlpNumCopy (urcoef[ur_freebeg + i], urcoef[rbeg + i]);
	...
	mpf_t val)
{
	...
	f->ucindx[cloc] = row;
	mpf_EGlpNumCopy (f->uccoef[cloc], val);
	f->ucrind[cloc] = rnzcnt;
	f->urindx[rloc] = col;
	mpf_EGlpNumCopy (f->urcoef[rloc], val);
	...
	int ind)
{
	...
	if (ind != nzcnt)
	{
		mpf_EGlpNumCopy (urcoef[rbeg + ind], urcoef[rbeg + nzcnt]);
	...
	int ind)
{
	...
	if (ind != nzcnt)
	{
		mpf_EGlpNumCopy (uccoef[cbeg + ind], uccoef[cbeg + nzcnt]);
	...
	mpf_svector * x)
{
	...
	{
		x->indx[i] = urindx[i];
		mpf_EGlpNumCopy (x->coef[i], urcoef[i]);
	...
	int *p_last_rank)
{
	...
	mpf_EGlpNumInitVar (max);
	mpf_EGlpNumCopy (max, f->maxelem_cur);
#endif /* TRACK_FACTOR */
	...
	for (i = 0; i < nzcnt; i++)
	{
		...
		CHECKRVALG (rval, CLEANUP);
#ifdef TRACK_FACTOR
		mpf_EGlpNumSetToMaxAbs (max, acoef[i]);
#endif /* TRACK_FACTOR */
		j = rrank[aindx[i]];
		if (j > last_rank)
			last_rank = j;
...
#ifdef TRACK_FACTOR
	f->nzcnt_cur += nzcnt;
	mpf_EGlpNumCopy (f->maxelem_cur, max);
	mpf_EGlpNumClearVar (max);
#endif /* TRACK_FACTOR */
	...
	int rank_r)
{
	...
	mpf_EGlpNumCopy (max, f->maxelem_cur);
#endif /* TRACK_FACTOR */
	mpf_EGlpNumInitVar (pivot_mul);

	for (i = rank_p; i < rank_r; i++)
	{
		c = cperm[i];
		if (mpf_EGlpNumIsNeqZero (work_coef[c], f->fzero_tol))	/*
																												 * if (work_coef[c] > fzero_tol || work_coef[c] < -fzero_tol) */
		{
			r = rperm[i];
			beg = ur_inf[r].rbeg;
			nzcnt = ur_inf[r].nzcnt;
			mpf_EGlpNumCopyFrac (pivot_mul, work_coef[c], urcoef[beg]);
			mpf_EGlpNumZero (work_coef[c]);
			for (j = 1; j < nzcnt; j++)
			{
				mpf_EGlpNumSubInnProdTo (work_coef[urindx[beg + j]], pivot_mul, urcoef[beg + j]);	/* 0.85 */
			}
			if (er_freebeg >= er_space)
			{
				...
				mpf_EGlpNumClearVar (max);
#endif
				mpf_EGlpNumClearVar (pivot_mul);
				return E_UPDATE_NOSPACE;
			}
			erindx[er_freebeg] = r;
			mpf_EGlpNumCopy (ercoef[er_freebeg], pivot_mul);
#ifdef TRACK_FACTOR
			mpf_EGlpNumSetToMaxAbs (max, pivot_mul);
#endif /* TRACK_FACTOR */
			er_freebeg++;
		}
		else
	...
	int minrank)
{
	...
	{
		if (mpf_EGlpNumIsNeqqZero (a[cperm[i]]))
		{
			j = cperm[i];
			if (mpf_EGlpNumIsNeqZero (a[j], f->fzero_tol))	/*
																									 * if (a[j] > fzero_tol || a[j] < -fzero_tol) */
			{
				rval = add_nonzero (f, row, j, a[j]);
				CHECKRVALG (rval, CLEANUP);
#ifdef TRACK_FACTOR
				mpf_EGlpNumSetToMaxAbs (max, a[j]);
#endif /* TRACK_FACTOR */
			}
			mpf_EGlpNumZero (a[j]);
		}
	...
	int rank_r)
{
	...
	do
	{
...
#ifdef TRACK_FACTOR
				mpf_EGlpNumSetToMaxAbs (f->maxelem_cur, v);
#endif /* TRACK_FACTOR */
				newr->indx[newr->nzcnt] = r;
				mpf_EGlpNumCopy (newr->coef[newr->nzcnt], v);
				newr->nzcnt++;
				mpf_EGlpNumClearVar (v);
				return 0;
			}
																						 ...
																						 * if (v > fzero_tol || v < -fzero_tol) */
		{
			...
			mpf_EGlpNumCopy (f->ercoef[f->er_freebeg], v);
#ifdef TRACK_FACTOR
			mpf_EGlpNumSetToMaxAbs (f->maxelem_cur, v);
#endif /* TRACK_FACTOR */
			f->er_freebeg++;
	...
	int rank_r)
{
	...
	for (i = 0; i < xnzcnt; i++)
	{
		...
		mpf_EGlpNumCopy (work[j], xcoef[i]);
	}

	newr.nzcnt = 0;
	ILL_SAFE_MALLOC (newr.indx, f->dim, int);

	newr.coef = mpf_EGlpNumAllocArray (f->dim);
...
CLEANUP:
	mpf_EGlpNumFreeArray (newr.coef);
	...
	int c)
{
	...
	if (urindx[beg] == c)
		return 0;
	mpf_EGlpNumInitVar (dt);

	for (i = 1; i < nzcnt; i++)
	{
		if (urindx[beg + i] == c)
		{
			mpf_EGLPNUM_SWAP (urcoef[beg], urcoef[beg + i], dt);
	...
	int r)
{
	...
	for (i = 1; i < nzcnt; i++)
	{
		if (ucindx[beg + i] == r)
		{
			mpf_EGLPNUM_SWAP (uccoef[beg], uccoef[beg + i], dt);
...
#ifdef DEBUG_FACTOR
	{
		QSlog("mpf_ILLfactor_update col %d:", col_p);
		for (i = 0; i < a->nzcnt; i++)
		{
			QSlog(" %.3f*%d", mpf_EGlpNumToLf (a->coef[i]), a->indx[i]);
		}
	}
#endif /* DEBUG_FACTOR */
...
#ifdef NOTICE_BLOWUP
	mpf_EGlpNumInitVar (tmpsize);
	mpf_EGlpNumSet (tmpsize, f->updmaxmult);
	mpf_EGlpNumMultTo (tmpsize, f->maxelem_orig);
	if (mpf_EGlpNumIsLess (tmpsize, f->maxelem_cur))
	{
/* Bico - comment out for dist 
