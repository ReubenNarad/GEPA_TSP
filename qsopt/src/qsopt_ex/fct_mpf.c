...
void mpf_ILLfct_zero_workvector (
	mpf_lpinfo * lp)
{
	int i;
...
void mpf_ILLfct_set_variable_type (
	mpf_lpinfo * lp)
{
	int j;
	...
	for (j = 0; j < lp->ncols; j++)
	{
		...
		case 0:
			if (mpf_EGlpNumIsLess (lp->lz[j], lp->uz[j]))
				lp->vtype[j] = VBOUNDED;
			else if (!mpf_EGlpNumIsNeqqZero (lp->lz[j]) &&
							 (lp->vclass[j] == CLASS_LOGICAL))
...
void mpf_ILLfct_compute_phaseI_xbz (
	mpf_lpinfo * lp)
{
	int i, j, r;
	int col, mcnt, mbeg;
...
void mpf_ILLfct_compute_phaseI_piz (
	mpf_lpinfo * lp)
{
	int i, r;
	mpf_svector *srhs = &(lp->srhs);
...
void mpf_ILLfct_compute_phaseI_dz (
	mpf_lpinfo * lp)
{
	int i, j;
	int col;
...
void mpf_ILLfct_compute_binvrow (
	mpf_lpinfo * lp,
	mpf_svector * zz,
	int row,
	mpf_t ztoler)
{
	...
	if (mpf_EGlpNumIsGreatZero (ztoler))
...
void mpf_ILLfct_compute_psteep_upv (
	mpf_lpinfo * lp,
	mpf_svector * swz)
{
...
void mpf_ILLfct_compute_dsteep_upv (
	mpf_lpinfo * lp,
	mpf_svector * swz)
{
	...
	mpf_svector * zA)
{
	if (z->nzcnt < lp->nrows / 2)
		return compute_zA3 (lp, z, zA, mpf_PIVZ_TOLER);
	else
		return compute_zA1 (lp, z, zA, mpf_PIVZ_TOLER);
}

/* compute v^T A */
void mpf_ILLfct_compute_vA (
	mpf_lpinfo * lp,
	mpf_svector * v,
	mpf_t * vA)
{
	...
	for (j = 0; j < lp->ncols; j++)
		if (!mpf_EGlpNumIsNeqZero (vA[j], mpf_SZERO_TOLER))
			mpf_EGlpNumZero (vA[j]);
...
*/
void mpf_ILLfct_update_basis_info (
	mpf_lpinfo * lp,
	int eindex,
	int lindex,
	int lvstat)
	...
	const mpf_t alpha)
{
	...
	if (!mpf_EGlpNumIsNeqqZero (alpha))
		return;
	...
	int *chgb)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		...
		{
			mpf_EGlpNumSet (newb, -1.0 * (my_rand (50, &(lp->rstate)) + 1.0));
		...
		{
			mpf_EGlpNumSet (newb, my_rand (50, &(lp->rstate)) + 1.0);
			mpf_EGlpNumMultTo (newb, cftol);
...
int mpf_ILLfct_adjust_viol_bounds (
	mpf_lpinfo * lp)
{
	int rval = 0;
	int chgb = 0;
...
int mpf_ILLfct_perturb_phaseI_bounds (
	mpf_lpinfo * lp)
{
	int rval = 0;
	int chgb = 0;
...
void mpf_ILLfct_unroll_bound_change (
	mpf_lpinfo * lp)
{
	int col;
	int changex = 0;
	...
	while (lp->nbchange != 0)
	{
		...
		if (lp->vtype[col] == VBOUNDED)
		{
			if (mpf_EGlpNumIsEqqual (lp->lz[col], lp->uz[col]))
				lp->vtype[col] = (!mpf_EGlpNumIsNeqqZero (lp->lz[col])) ?
					VARTIFICIAL : VFIXED;
		}
	...
	int *chgc)
{
	...
	for (i = 0; i < lp->nnbasic; i++)
	{
		...
		case STAT_LOWER:
			...
			{
				mpf_EGlpNumSet (newc, my_rand (50, &(lp->rstate)) + 1.0);
				mpf_EGlpNumMultTo (newc, cftol);
				mpf_EGlpNumAddTo (newc, *c);
				if (mpf_EGlpNumIsLessZero (*dj))
...
int mpf_ILLfct_adjust_viol_coefs (
	mpf_lpinfo * lp)
{
	int rval = 0;
	int chgc = 0;
...
void mpf_ILLfct_unroll_coef_change (
	mpf_lpinfo * lp)
{
	int bascoef = 0;
...
void mpf_ILLfct_check_pfeasible (
	mpf_lpinfo * lp,
	mpf_feas_info * fs,
	const mpf_t ftol)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
				...
				&& mpf_EGlpNumIsNeq (lp->uz[col], mpf_INFTY, mpf_oneLpNum))
		{
			...
			WARNINGL (QSE_WLVL, mpf_EGlpNumIsLess (mpf_INFTY, err1),
						 ...
						 && mpf_EGlpNumIsNeq (lp->lz[col], mpf_NINFTY, mpf_oneLpNum))
		{
			...
			WARNINGL (QSE_WLVL, mpf_EGlpNumIsLess (mpf_INFTY, err2),
	...
	if (mpf_EGlpNumIsNeqqZero (infeas))
	{
		...
		if (mpf_EGlpNumIsLessZero (fs->totinfeas))
		{
...
/* feasibility routines */
void mpf_ILLfct_check_pIpfeasible (
	mpf_lpinfo * lp,
	mpf_feas_info * fs,
	mpf_t ftol)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (!mpf_EGlpNumIsNeqZero (lp->xbz[i], ftol))
			continue;
		col = lp->baz[i];
		if (mpf_EGlpNumIsGreatZero(lp->xbz[i]) &&
				mpf_EGlpNumIsNeqq (lp->uz[col], mpf_INFTY))
		{
			ninf++;
		}
		else if (mpf_EGlpNumIsLessZero (lp->xbz[i]) &&
...
void mpf_ILLfct_check_dfeasible (
	mpf_lpinfo * lp,
	mpf_feas_info * fs,
	const mpf_t ftol)
{
	...
	{
		lp->dfeas[j] = 0;
		if (!mpf_EGlpNumIsNeqZero (lp->dz[j], ftol))
			continue;
		col = lp->nbaz[j];
...
void mpf_ILLfct_check_pIdfeasible (
	mpf_lpinfo * lp,
	mpf_feas_info * fs,
	mpf_t ftol)
{
	...
	for (j = 0; j < lp->nnbasic; j++)
	{
		if (!mpf_EGlpNumIsNeqZero (dz[j], ftol))
			continue;
		col = lp->nbaz[j];
		if (lp->vtype[col] == VARTIFICIAL || lp->vtype[col] == VFIXED)
			continue;

		if (mpf_EGlpNumIsLessZero (dz[j]) &&
				(lp->vstat[col] == STAT_LOWER || lp->vstat[col] == STAT_ZERO))
			ninf++;
		else if (mpf_EGlpNumIsGreatZero (dz[j]) &&
						 (lp->vstat[col] == STAT_UPPER || lp->vstat[col] == STAT_ZERO))
			ninf++;
	}
...
void mpf_ILLfct_dphaseI_simple_update (
	mpf_lpinfo * lp,
	mpf_t ftol)
{
	int j, col;
	...
	for (j = 0; j < lp->nnbasic; j++)
	{
		if (!mpf_EGlpNumIsNeqZero (lp->dz[j], ftol))
			continue;
		col = lp->nbaz[j];
		if (mpf_EGlpNumIsLessZero (lp->dz[j] ) && lp->vtype[col] == VBOUNDED)
			lp->vstat[col] = STAT_UPPER;
		else if (mpf_EGlpNumIsGreatZero (lp->dz[j]) && lp->vtype[col] == VBOUNDED)
			lp->vstat[col] = STAT_LOWER;
	}
}

/* set status values */
void mpf_ILLfct_set_status_values (
	mpf_lpinfo * lp,
	int pstatus,
	int dstatus,
	int ptype,
...
#define C_VALUE(a) (1.0+(double)(a)/(PARAM_HEAP_RATIO*ILLutil_our_log2(a)))
	mpf_EGlpNumSet (c->y_ravg, C_VALUE (lp->nrows));
	mpf_EGlpNumSet (c->za_ravg, C_VALUE (lp->nnbasic));
...
void mpf_ILLfct_update_ppI_prices (
	mpf_lpinfo * lp,
	mpf_price_info * pinf,
	mpf_svector * srhs,
	mpf_svector * ssoln,
	...
	mpf_svector * srhs)
{
	...
	for (j = 0; j < tctr && mpf_EGlpNumIsLeq (t[perm[j]], tz); j++)
	{
		...
		if (lp->iwork[c] != 1)
		{
			...
			if (cbnd == BSKIP)
			{
				if (!mpf_EGlpNumIsNeqZero (dj, *dftol));
				else if (mpf_EGlpNumIsLessZero (dj) && vs == STAT_LOWER)
					lp->vstat[col] = STAT_UPPER;
				else if (mpf_EGlpNumIsGreatZero (dj) && vs == STAT_UPPER)
					lp->vstat[col] = STAT_LOWER;
			}
			else if (c != eindex)
			{
				if (!mpf_EGlpNumIsNeqZero (dj, *dftol))
					f = 0;
				else if (mpf_EGlpNumIsLessZero (dj) &&
								 (vs == STAT_LOWER || vs == STAT_ZERO))
					f = -1;
				else if (mpf_EGlpNumIsGreatZero (dj) &&
...
void mpf_ILLfct_update_dpI_prices (
	mpf_lpinfo * lp,
	mpf_price_info * pinf,
	mpf_svector * srhs,
	mpf_svector * ssoln,
...
void mpf_ILLfct_update_dIIfeas (
	mpf_lpinfo * lp,
	int eindex,
	mpf_svector * srhs)
{
...
void mpf_ILLfct_update_dpII_prices (
	mpf_lpinfo * lp,
	mpf_price_info * pinf,
	mpf_svector * srhs,
	mpf_svector * ssoln,
	...
	mpf_t alpha)
{
	...
	else
	{
		...
		mpf_ILLfct_update_xz (lp, alpha, -1, -1);
		add_vectors (lp, ssoln, &(lp->yjz), ssoln, mpf_oneLpNum);
		u = ssoln;
	}
	...
	mpf_price_info * p)
{
	...
	if (mpf_EGlpNumIsNeqqZero (err))
		QSlog("dI x err = %.7f, ern = %d", mpf_EGlpNumToLf (err), ern);
	...
	mpf_price_info * p)
{
	...
	if (mpf_EGlpNumIsNeqqZero (err))
		QSlog("dII x err = %.7f, ern = %d", mpf_EGlpNumToLf (err), ern);
	...
	mpf_price_info * p)
{
	...
	if (mpf_EGlpNumIsNeqqZero (err))
		QSlog("pI pi err = %.7f, ern = %d", mpf_EGlpNumToLf (err), ern);
	...
	if (mpf_EGlpNumIsNeqqZero (err))
		QSlog("pI dz err = %.7f, ern = %d", mpf_EGlpNumToLf (err), ern);
	...
	mpf_price_info * p)
{
	...
	if (mpf_EGlpNumIsNeqqZero (err))
		QSlog("pII pi err = %.7f, ern = %d", mpf_EGlpNumToLf (err), ern);
	...
	if (mpf_EGlpNumIsNeqqZero (err))
		QSlog("pII dz err = %.7f, ern = %d", mpf_EGlpNumToLf (err), ern);
	/*
