	...
	mpq_heap * h)
{
	...
	QSlog("\n key: ");
	for (i = 0; i < h->maxsize; i++)
		QSlog("%la ", mpq_EGlpNumToLf (h->key[i]));
	QSlog("\n key(sorted): ");
	for (i = 0; i < h->size; i++)
		QSlog("%la ", mpq_EGlpNumToLf (h->key[h->entry[i]]));
}
	...
	mpq_heap * h)
{
	...
	{
		if (h->loc[i] < -1)
			QSlog("error in mpq_heap\n");
		else if (h->loc[i] > -1)
			tcnt++;
	}
	if (tcnt != h->size)
		QSlog("error 3 in mpq_heap\n");

	for (i = 0; i < h->size; i++)
	{
		if (h->loc[h->entry[i]] != i)
			QSlog("error 1 in mpq_heap\n");
		if (!mpq_EGlpNumIsNeqqZero (h->key[h->entry[i]]))
			QSlog("error 2 in mpq_heap\n");
		if (mpq_EGlpNumIsLess (h->key[h->entry[HEAP_UP (i)]], h->key[h->entry[i]]))
			QSlog("error 4 in mpq_heap\n");
	}
}
	...
	mpq_t * key)
{
	...
	for (i = 0; i < nelems; i++)
	{
		if (mpq_EGlpNumIsGreatZero (key[i]))
		{
			h->entry[n] = i;
			h->loc[i] = n;
			n++;
	...
	mpq_ILLmatrix * A)
{
	...
	else
	{
		...
		for (j = 0; j < A->matcols; j++)
		{
			...
			{
				EGioPrintf (fd, "row %d=%.3f ", A->matind[k], mpq_EGlpNumToLf (A->matval[k]));
			}
			EGioPrintf (fd, "\n");
		}
	}
