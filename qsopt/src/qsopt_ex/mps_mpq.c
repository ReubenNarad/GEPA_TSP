	...
	mpq_rawlpdata * lp)
{
	...
	if (skip)
	{
		mpq_ILLmps_set_end_of_line (state);	/* to avoid warning about extra fields */
	}
	else
	{
		if (strcmp (rhsname, " "))
	...
	mpq_qserror_collector * collector)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		...
		{
			str = mpq_EGlpNumGetStr(lp->rhs[i]);
			mpq_ILLprint_report (lp, " RHS    %s    %s\n", rownames[i], str);
			EGfree(str);
		}
	}
	...
	if (lp->rangeval)
	{
		...
		for (i = 0; i < lp->nrows; i++)
		{
			...
			{
				str = mpq_EGlpNumGetStr(lp->rangeval[i]);
				mpq_ILLprint_report (lp, " RANGE    %s    %s\n", rownames[i], str);
				EGfree(str);
			}
		}
	}
	...
	if (ri != lp->nstruct)
	{
		...
		for (; ri < lp->nstruct; ri++)
		{
			...
			{
				str = mpq_EGlpNumGetStr(lp->lower[i]);
				mpq_ILLprint_report (lp, " FX BOUND    %s    %s\n", colnames[ri], str);
				EGfree(str);
				continue;
			}
			...
			prtUpper = !mpq_ILLraw_default_upper (lp, i, ri);
			if (prtLower)
			{
				if (mpq_EGlpNumIsEqqual (lp->lower[i], mpq_ILL_MINDOUBLE))
				{
				...
				{
					str = mpq_EGlpNumGetStr(lp->lower[i]);
					mpq_ILLprint_report (lp, " LO BOUND    %s    %s\n", colnames[ri], str);
					EGfree(str);
				}
			}
			if (prtUpper)
			{
				...
				{
					str = mpq_EGlpNumGetStr(lp->upper[i]);
					mpq_ILLprint_report (lp, " UP BOUND    %s    %s\n", colnames[ri], str);
					EGfree(str);
				}
			}
		}
	...
	char *objname)
{
	...
	{
		str = mpq_EGlpNumGetStr(lp->obj[i]);
		mpq_ILLprint_report (lp, "  %s    %s    %s\n", colname, objname, str);
		EGfree(str);
	}
	for (k = A->matbeg[i]; k < A->matbeg[i] + A->matcnt[i]; k++)
	{
		row = A->matind[k];
		str = mpq_EGlpNumGetStr(A->matval[k]);
