...
#include "qsopt_mpq.h"
#include "lib_mpq.h"								/* for mpq_ILLlib_writebasis */
#include "lp_mpq.h"									/* for mpq_ILLwrite_lp */
...
void mpq_ILLsimplex_init_lpinfo (
	mpq_lpinfo * lp)
{
...
void mpq_ILLsimplex_free_lpinfo (
	mpq_lpinfo * lp)
{
	if (lp)
	{
...
void mpq_ILLsimplex_load_lpinfo (
	mpq_ILLlpdata * qslp,
	mpq_lpinfo * lp)
{
	lp->basisid = -1;
	...
	mpq_lpinfo * lp)
{
	...
	if (mpq_EGlpNumIsNeqqZero (lp->tol->ip_tol))
	{
		...
		MESSAGE (VERBOSE_LEVEL, "ip_tol %lg", mpq_EGlpNumToLf (lp->tol->ip_tol));
		MESSAGE (VERBOSE_LEVEL, "eps %lg", mpq_EGlpNumToLf (mpq_epsLpNum));
		MESSAGE (VERBOSE_LEVEL, "mpq_PFEAS_TOLER %lg", mpq_EGlpNumToLf (mpq_PFEAS_TOLER));
#endif
	...
	{
#if VERBOSE_LEVEL <= DEBUG
		MESSAGE (VERBOSE_LEVEL, "id_tol %lg", mpq_EGlpNumToLf (lp->tol->id_tol));
#endif
		mpq_EGlpNumDivUiTo (lp->tol->id_tol, 2UL);
	}
}

void mpq_init_internal_lpinfo (
	mpq_lpinfo * lp)
{
	int rval = 0;
	...
	mpq_lpinfo * lp)
{
	...
	{
		QSlog("mpq_build_internal_lpinfo");
		rval = 1;
		goto CLEANUP;
	}
...
int mpq_ILLsimplex_retest_psolution (
	mpq_lpinfo * lp,
	mpq_price_info * p,
	int phase,
	mpq_feas_info * fi)
{
	...
	mpq_t *dtol = &(lp->tol->dfeas_tol);
	mpq_t *iptol = &(lp->tol->ip_tol);
	mpq_t *idtol = &(lp->tol->id_tol);
	...
	if (phase == PRIMAL_PHASEII)
	{
		...
		mpq_ILLfct_check_dfeasible (lp, fi, *dtol);
	}
	else if (phase == PRIMAL_PHASEI)
	{
		...
		if (fi->pstatus != PRIMAL_FEASIBLE)
		{
			if (lp->pIpiz)
			{
				...
				mpq_ILLfct_check_pIdfeasible (lp, fi, *idtol);
				if (p != NULL && p->p_strategy == COMPLETE_PRICING)
...
int mpq_ILLsimplex_retest_dsolution (
	mpq_lpinfo * lp,
	mpq_price_info * p,
	int phase,
	mpq_feas_info * fi)
{
	...
	{
		mpq_ILLfct_check_dfeasible (lp, fi, *idtol);
		if (fi->dstatus != DUAL_FEASIBLE)
		{
			mpq_ILLfct_compute_phaseI_xbz (lp);
			mpq_ILLfct_check_pIpfeasible (lp, fi, *iptol);
			if (p != NULL)
			{
				if (p->d_strategy == COMPLETE_PRICING)
...
int mpq_ILLsimplex_infcertificate (
	mpq_lpinfo * lp,
	mpq_t * pi)
{
	int i, col, nz;
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (sense[i] == 'G' && mpq_EGlpNumIsLessZero (pi[i]))
			mpq_EGlpNumZero (pi[i]);
		if (sense[i] == 'L' && mpq_EGlpNumIsGreatZero (pi[i]))
			mpq_EGlpNumZero (pi[i]);
	}
	return 0;
}
	...
	mpq_t * pi)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (lp->O->sense[i] == 'G' && mpq_EGlpNumIsLessZero (pi[i]))
			QSlog("compl");
		if (lp->O->sense[i] == 'L' && mpq_EGlpNumIsGreatZero (pi[i]))
			QSlog("compll");
	}
	...
	itcnt_t*itcnt)
{
	...
	if (it.sdisplay)
	{
		...
		while (i--)
			nonzero += lp->matcnt[i];
		sprintf (buffer, "starting mpq_ILLsimplex on %s...", lp->O->probname);
	...
	if (B != 0)
	{
		...
		{
			QSlog("Unknown algorithm %d in mpq_ILLsimplex", it.algorithm);
			rval = 1;
			ILL_CLEANUP;
		}
	}
...
START:
	...
	if (it.algorithm == DUAL_SIMPLEX)
	{
		if (B != NULL || it.resumeid == SIMPLEX_RESUME_UNSHIFT)
			mpq_ILLfct_dual_adjust (lp, lp->tol->dfeas_tol);
		else
			mpq_ILLfct_dual_adjust (lp, mpq_zeroLpNum);
	}
	mpq_ILLfct_compute_xbz (lp);

	mpq_ILLfct_check_pfeasible (lp, &fi, lp->tol->pfeas_tol);
	mpq_ILLfct_check_dfeasible (lp, &fi, lp->tol->dfeas_tol);
...
TERMINATE:
	...
	if (it.sdisplay)
	{
		QSlog("completed mpq_ILLsimplex");
	...
	if (status)
	{
		...
		{
			QSlog("unknown solution status in mpq_ILLsimplex %d",
									it.solstatus);
			rval = 1;
			CHECKRVALG (rval, CLEANUP);
		}
   ...
   /* end update */
	if (it.sdisplay)
	{
		...
		{
		case OPTIMAL:
			QSlog("opt = %f", mpq_EGlpNumToLf (lp->objval));
			break;
		...
		case PRIMAL_FEASIBLE:
			QSlog("primal obj = %f", mpq_EGlpNumToLf (lp->pobjval));
			break;
		...
		case DUAL_UNBOUNDED:
			...
			break;
		case DUAL_FEASIBLE:
			QSlog("dual obj = %f", mpq_EGlpNumToLf (lp->dobjval));
			break;
		}
	...
	mpq_t prevobj)
{
	...
	if (!mpq_EGlpNumIsNeqZero (denom, mpq_PROGRESS_THRESH))
	{
	...
	int phase)
{
	...
	case PRIMAL_PHASEI:
		...
		mpq_EGlpNumAddTo (fi.totinfeas, lp->pinfeas);
		strcpy (print_str, "primal infeas");
		if (mpq_EGlpNumIsLessZero (lp->pinfeas) &&
				(mpq_EGlpNumIsNeqZero (lp->pinfeas, mpq_oneLpNum)))
		{
			/*QSlog("Negative Infeasibility! Imposible %lg %la, iter %d",
			 * mpq_EGlpNumToLf (print_val), mpq_EGlpNumToLf (print_val), it->itercnt);
			 */
			//exit(1);
		}
		break;
	...
	case DUAL_PHASEI:
		...
		mpq_EGlpNumAddTo (fi.totinfeas, lp->dinfeas);
	...
	mpq_EGlpNumAddUiTo (fi.totinfeas, 1000);
	if (mpq_EGlpNumIsLessZero (fi.totinfeas))
	{
		...
		if (mpq_EGlpNumIsLess (it->objtol, print_val))
		{
			...
			if (fi.dstatus == DUAL_FEASIBLE)
			{
				...
				if (mpq_EGlpNumIsLess (lp->objbound, lp->dobjval))
				{
					...
					/*if (it->sdisplay) */
					{
						QSlog("bound reached %lf %lf", mpq_EGlpNumToLf (lp->objbound),
										mpq_EGlpNumToLf (lp->dobjval));
					}
				}
				else
	...
	if (pr.price_stat == PRICE_OPTIMAL)
	{
		...
		if (lp->nbchange != 0)
		{
			...
			mpq_ILLfct_set_status_values (lp, fi.pstatus, -1, PHASEII, -1);
			 ...
			 /*HHH*/ mpq_ILLfct_check_dfeasible (lp, &fi, lp->tol->dfeas_tol);
	...
	mpq_iter_info * it)
{
	...
	if (it->newphase != 0)
	{
		mpq_ILLfct_check_dfeasible (lp, &fi, lp->tol->id_tol);
		if (it->newphase == SIMPLEX_PHASE_NEW)
		{
			it->noprog = 0;
			if (it->sdisplay)
	...
	mpq_iter_info * it)
{
	...
	if (pr.price_stat == PRICE_OPTIMAL)
	{
		...
		if (it->sdisplay > 1)
		{
			//ILL_IFTRACE("\t%s:%s:%d\n",__func__,__FILE__,__LINE__);
			QSlog("problem seemingly solved");
			QSlog("seemingly dual opt = %f", mpq_EGlpNumToLf (lp->dobjval));
			QSlog("retesting soln");
		}
	...
	else if (rs.ratio_stat == RATIO_BCHANGE)
	{
		...
		mpq_ILLfct_update_dpII_prices (lp, pinf, &(lp->srhs), &(lp->ssoln), /*rs.eindex,*/
	...
	int *basis_mod)
{
	...
	*basis_mod = 0;
	...
	/* QSlog("Forcing vars into basis in mpq_ILLsimplex_pivotin"); */
	mpq_ILLsvector_init (&wz);
	rval = mpq_ILLsvector_alloc (&wz, lp->nrows);
	CHECKRVALG (rval, CLEANUP);
	mpq_ILLsvector_init (&updz);
	...
	mpq_t value)
{
	...
	if (it->sdisplay && it->itercnt % lp->iterskip == 0)
	{
		...
		snprintf (buffer, (size_t) 1023, "(%d): %s = %10.7lf", it->itercnt,
							value_name, mpq_EGlpNumToLf (value));
