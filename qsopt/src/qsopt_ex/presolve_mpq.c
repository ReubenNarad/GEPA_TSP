...
#define ILL_PRE_FEAS_TOL mpq_PFEAS_TOLER	//(1e-6)
#define ILL_PRE_ZERO_TOL mpq_PIVOT_TOLER	//(1e-10)
	...
	mpq_ILLlpdata * lp)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlp_add_logicals called with a NULL pointer");
		rval = 1;
		goto CLEANUP;
	}

	QSlog("mpq_ILLlp_add_logicals ...");
	...
	for (i = 0; i < nrows; i++)
	{
		...
		case 'R':									/* Range     */
			...
			break;
		default:
			QSlog("unknown sense %c in mpq_ILLlp_add_logicals", sense[i]);
			rval = 1;
			goto CLEANUP;
		}
		ncols++;
	...
	mpq_ILLlpdata * lp)
{
	...
	for (j = 0; j < nstruct; j++)
	{
		...
		{
			row = A->matind[k];
			if (mpq_EGlpNumIsGreatZero (gama[row]))
			{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (mpq_EGlpNumIsGreatZero ( gama[i]))
		{
	...
	int pre_types)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlp_presolve called with a NULL pointer");
		rval = 1;
		goto CLEANUP;
	}
	...
	mpq_ILLlp_preline * line)
{
	...
	if (line->row_or_col == 0)
	{
		...
		{
			QSlog(" C%d->%g", line->ind[k], mpq_EGlpNumToLf (line->val[k]));
		}
		QSlog(" RHS->%g", mpq_EGlpNumToLf (line->rhs));
	}
	else
	{
		for (k = 0; k < line->count; k++)
		{
			QSlog(" R%d->%g", line->ind[k], mpq_EGlpNumToLf (line->val[k]));
		}
		QSlog(" Obj->%g  LB->%g  UB->%g", mpq_EGlpNumToLf (line->obj),
	...
	for (j = 0; j < ncols; j++)
	{
		if (cols[j].del == 0)
		{
			...
			if (k == cols[j].deg)
			{
				...
				if (!mpq_EGlpNumIsNeqZero (objtmp, ILL_PRE_FEAS_TOL))
				{
					set_fixed_variable (G, j, cols[j].lower);
				}
				else if (mpq_EGlpNumIsGreatZero (objtmp))
				{
					if (mpq_EGlpNumIsEqqual (cols[j].lower, mpq_ILL_MINDOUBLE))
					{
						QSlog("unbounded prob detected in empty_columns");
						QSlog("col %d, obj %g", j, mpq_EGlpNumToLf (cols[j].obj));
						rval = 1;
						goto CLEANUP;
					}
					else
				...
				else if (mpq_EGlpNumIsLessZero (objtmp))
				{
	...
	int *hit)
{
	...
	*hit = 0;
	...
	while (list)
	{
		...
		if (tdeg[rowindex] == 0)
		{
			if (mpq_EGlpNumIsNeqZero (r->rhs, ILL_PRE_FEAS_TOL))
			{
				QSlog("infeasible row detected in singleton_row");
				QSlog("empty row with rhs = %g", mpq_EGlpNumToLf (r->rhs));
				rval = 1;
				goto CLEANUP;
			}
		...
		else
		{
			...
			if (mpq_EGlpNumIsSumLess (val, ILL_PRE_FEAS_TOL, c->lower) ||
					mpq_EGlpNumIsSumLess (c->upper, ILL_PRE_FEAS_TOL, val))
			{
	...
	*hit = 0;
	...
	for (i = 0; i < nrows; i++)
	{
		if (rows[i].del == 0)
		{
			...
			if (mpq_EGlpNumIsSumLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ||
					mpq_EGlpNumIsSumLess (ub, ILL_PRE_FEAS_TOL, rows[i].rhs))
			{
			...
			else if (mpq_EGlpNumIsDiffLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ||
							 mpq_EGlpNumIsDiffLess (ub, ILL_PRE_FEAS_TOL, rows[i].rhs))
			{
				(*hit)++;
				ts = (mpq_EGlpNumIsDiffLess (rows[i].rhs, ILL_PRE_FEAS_TOL, lb) ? 0 : 1);
				for (k = 0; k < rows[i].deg; k++)
				{
					e = rows[i].adj[k];
					if (e->del == 0)
					{
						...
						if ((ts == 0 && mpq_EGlpNumIsLessZero (e->coef)) ||
								(ts == 1 && mpq_EGlpNumIsGreatZero (e->coef)))
						{
							set_fixed_variable (G, j, cols[j].upper);
						}
						else
	...
	int *hit)
{
	...
	*hit = 0;
	...
	for (i = 0; i < nrows; i++)
	{
		...
		{
			s[i] = mpq_ILL_MAXINT;				/* mpq_ILL_MAXINT means no longer eligible    */
		}
		else
		{
			...
			else if (s[ri] < t0)
			{
				...
				for (k2 = k + 1; k2 < c->deg; k2++)
				{
					...
					if (s[i] == s[ri])
					{
						...
						if (mpq_EGlpNumIsEqual (q, mpq_oneLpNum, ILL_PRE_ZERO_TOL))
						{
							s[ri] = t;
							s[i] = t;
							got++;
	...
	int *hit)
{
	...
	*hit = 0;
	...
	for (j = 0; j < ncols; j++)
	{
		...
		{
			s[j] = mpq_ILL_MAXINT;				/* mpq_ILL_MAXINT means no longer eligible    */
		}
		else
		{
	...
	mpq_t * ub)
{
	...
	for (k = 0; k < rows[i].deg; k++)
	{
		...
		if (e->del == 0)
		{
			if (mpq_EGlpNumIsLessZero (e->coef))
			{
			...
			else if (mpq_EGlpNumIsGreatZero (e->coef))
			{
	...
	mpq_t * ub)
{
	...
	if (mpq_EGlpNumIsLess (ILL_PRE_FEAS_TOL, a_ij->coef))
	{
		if (mpq_EGlpNumIsLess (u, mpq_ILL_MAXDOUBLE))
		{
	...
	else if (mpq_EGlpNumIsLess (a_ij->coef, ILL_PRE_FEAS_TOL))
	{
		if (mpq_EGlpNumIsLess (mpq_ILL_MINDOUBLE, l))
		{
	...
	graph * G)
{
	...
	for (i = 0; i < G->nrows; i++)
	{
		...
		for (k = 0; k < G->rows[i].deg; k++)
		{
			...
			if (G->rows[i].adj[k]->coltype == ILL_PRE_COL_LOGICAL)
				QSlog("S");
			QSlog("(%g)", mpq_EGlpNumToLf (G->rows[i].adj[k]->coef));
		}
		QSlog("  rhs: %g", mpq_EGlpNumToLf (G->rows[i].rhs));
	...
	for (j = 0; j < G->ncols; j++)
	{
		...
		QSlog("  obj: %g  bnd: (%g, %g)", mpq_EGlpNumToLf (G->cols[j].obj),
	...
	mpq_ILLlp_sinfo * s)
{
	...
	if (!sense)
	{
		QSlog("out of memory in mpq_ILLlp_sinfo_print");
		rval = 1;
		goto CLEANUP;
	}
	for (i = 0; i < s->nrows; i++)
