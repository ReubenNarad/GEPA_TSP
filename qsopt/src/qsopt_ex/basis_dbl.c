	...
	int strict)
{
	...
	if (lp->basisid == -1)
	{
		QSlog("dbl_ILLbasis_tableau_row: no basis");
	...
	if (rhs != NULL && trow != NULL)
	{
		...
		if (dbl_EGlpNumIsLess (dbl_PIVZ_TOLER, coef))
			QSlog("tableau: bas computed = %.12f", dbl_EGlpNumToLf (sum));
		if (!strict)
...
#if BASIS_STATS > 0
	QSlog("b1: nz %d pinf %.2f dinf %.2f", nz1, dbl_EGlpNumToLf (pinf1),
							dbl_EGlpNumToLf (dinf1));
	QSlog("b2: nz %d pinf %.2f dinf %.2f", nz2, dbl_EGlpNumToLf (pinf2),
...
/* Bico - comment out for dist
       QSlog("Warning: numerically bad basis in dbl_ILLfactor_update");
*/
		*refactor = 1;
		rval = 0;
	}
