...
/*  EXPORTED FUNCTIONS                                                      */
/*                                                                          */
/*    int mpq_ILLlib_optimize (mpq_lpinfo *lp, mpq_ILLlp_basis *B, mpq_price_info *pinf,    */
...
/*    int mpq_ILLlib_get_slack (mpq_lpinfo *lp, mpq_ILLlp_cache *C, double *slack)      */
...
/*    int mpq_ILLlib_newrows (mpq_lpinfo *lp, mpq_ILLlp_basis *B, int num, double *rhs, */
...
/*    int mpq_ILLlib_addrow (mpq_lpinfo *lp, mpq_ILLlp_basis *B, int cnt, int *ind,     */
...
/*            const char **names, int factorok)                             */
/*    int mpq_ILLlib_delcols (mpq_lpinfo *lp, mpq_ILLlp_basis *B, int num, int *dellist */
/*            int *basis_ok)                                                */
...
/*    int mpq_ILLlib_getrows (mpq_lpinfo *lp, int num, int *rowlist, int **rowcnt,  */
/*            int **rowbeg, int **rowind, double **rowval, double **rhs,    */
/*            char **sense, char ***names)                                  */
/*    int mpq_ILLlib_getcols (mpq_lpinfo *lp, int num, int *collist, int **colcnt,  */
...
/*    int mpq_ILLlib_readbasis (mpq_lpinfo *lp, mpq_ILLlp_basis *B, char *fname)        */
/*    int mpq_ILLlib_writebasis (mpq_lpinfo *lp, const char *fname)                 */
...
/*    int mpq_ILLlib_print_x (EGioFile_t *fd, mpq_lpinfo *lp, mpq_ILLlp_cache *C, double *x,  */
/*            int nonZerosOnly)                                             */
	...
	itcnt_t*itcnt)
{
	...
	/* mpq_ILLprice_free_pricing_info (pinf); *//* Should be removed later */

	rval = mpq_ILLsimplex (lp, algo, B, pinf, &sol_status, simplex_display, itcnt);
	...
	mpq_t * rc)
{
	...
	if (C)
	{
		...
		{
			QSlog("cache mismatch in mpq_ILLlib_solution");
			rval = 0;
			ILL_CLEANUP;
		}
		if (val)
	...
	mpq_t * tabrow)
{
	...
	{
		QSlog("mpq_ILLlib_tableau called with bad row: %d", row);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	const mpq_t bnd)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_chgbnd called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	{
		QSlog("mpq_ILLlib_chgbnd called with bad indx: %d", indx);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	case 'B':
		...
		break;
	default:
		QSlog("mpq_ILLlib_chgbnd called with lu: %c", lu);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_t * bnd)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getbnd called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (indx < 0 || indx > lp->O->nstruct)
	{
		QSlog("mpq_ILLlib_getbnd called with bad indx: %d", indx);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	case 'U':
		...
		break;
	default:
		QSlog("mpq_ILLlib_getbnd called with lu: %c", lu);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_t *upper)
{
    ...
    if (!lp) {
        QSlog("mpq_ILLlib_getbnds_list called without an lp");
        rval = 1; ILL_CLEANUP;
    }
    ...
    nstruct = qslp->nstruct;
		...
		if(collist[j]<0|| collist[j] >= nstruct)
			{
				QSlog("mpq_ILLlib_getbnds_list collist[%d] = %d out "
	...
	itcnt_t*itcnt)
{
	...
	{
		lp->basisid = -1;
		rval = mpq_ILLlib_optimize (lp, 0, &lpinf, DUAL_SIMPLEX, &status, 0, itcnt);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < ncand; i++)
	{
		...
		rval = mpq_ILLlib_optimize (lp, &B, &lpinf, DUAL_SIMPLEX, &status, 0, itcnt);
		CHECKRVALG (rval, CLEANUP);
	...
	lp->maxiter = olditer;
	rval = mpq_ILLlib_optimize (lp, &origB, pinf, DUAL_SIMPLEX, &status, 0, itcnt);
	CHECKRVALG (rval, CLEANUP);
	...
	const char *name)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_addrow called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	{
		mpq_EGlpNumReallocArray (&(qslp->rhs), qslp->rowsize + EXTRA_ROWS);
		...
		if (qslp->rangeval || sense == 'R')
			mpq_EGlpNumReallocArray (&(qslp->rangeval), qslp->rowsize + EXTRA_ROWS);
	...
	{
		mpq_EGlpNumReallocArray (&(qslp->lower), qslp->colsize + EXTRA_COLS);
		mpq_EGlpNumReallocArray (&(qslp->upper), qslp->colsize + EXTRA_COLS);
		mpq_EGlpNumReallocArray (&(qslp->obj), qslp->colsize + EXTRA_COLS);
	...
	int *cache_ok)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_delrows called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
...
*/
		...
		if (bok == 1)
		{
			...
			if (B->rownorms)
			{
				...
				if (k != nrows - num)
				{
					QSlog("error in  mpq_ILLlib_delrows");
					rval = 1;
					ILL_CLEANUP;
				}
			}
	...
	int *basis_ok)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_delcols called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_t*val)
{
	...
	for (i = A->matbeg[col]; i < A->matbeg[col] + A->matcnt[col]; i++)
	{
		if (A->matind[i] == row)
		{
			mpq_EGlpNumCopy(*val, A->matval[i]);
			ILL_CLEANUP;
		}
	}
...
{ 
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_chgcoef called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	{
		QSlog("mpq_ILLlib_getcoef called with out-of-range index");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_t coef)
{
	...
	if (rowindex < 0 || rowindex >= nrows || colindex < 0 || colindex >= nstruct)
	{
		QSlog("mpq_ILLlib_chgcoef called with out-of-range index");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	char *sense)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		case 'R':									/* Range constraint, we will set its upper bound
																 once we call mpq_QSchange_range, by default it 
		...
		case 'L':									/* Slack     */
			...
			break;
		default:
			QSlog("illegal sense %c in mpq_ILLlib_chgsense", sense[i]);
			rval = 1;
			ILL_CLEANUP;
		}
	}
	...
	int factorok)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_addcol called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	const mpq_t * rowval)
{
	...
	newval = mpq_EGlpNumAllocArray (A->matsize + rowcnt + EXTRA_MAT);
	...
	mpq_t * colval)
{
	...
	if (A->matfree < colcnt + 1)
	{
		...
		//                                 sizeof (int));
		//CHECKRVALG(rval,CLEANUP);
		mpq_EGlpNumReallocArray (&(A->matval), A->matsize + colcnt + EXTRA_MAT + 1);
	...
	char ***names)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getrows called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (range)
	{
		*range = mpq_EGlpNumAllocArray(num);
		if(qslp->rangeval)
		{
			for(i = 0; i < num ; i++)
			{
				mpq_EGlpNumCopy((*range)[i], qslp->rangeval[rowlist[i]]);
			}
		}
		else
		{
	...
	char ***names)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getcols called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_t* obj)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getobj_list called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	for (j = 0; j < num; j++)
	{
		...
		{
			QSlog("mpq_ILLlib_getobj_list collist[%d] = %d outside"
									" valid range", j, col);
			rval = 1;
			ILL_CLEANUP;
		}
		mpq_EGlpNumCopy(obj[j],qslp->obj[structmap[col]]);
	}  
	...
	mpq_t * obj)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getobj called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	for (j = 0; j < nstruct; j++)
	{
		mpq_EGlpNumCopy (obj[j], qslp->obj[structmap[j]]);
	}
	...
	mpq_t coef)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_chgobj called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (indx < 0 || indx >= lp->O->nstruct)
	{
		QSlog("mpq_ILLlib_chgrhs called with bad indx: %d", indx);
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_t * rhs)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getrhs called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_t coef)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_chgrhs called without an lp");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	mpq_EGlpNumCopy(qslp->rangeval[indx], coef);
	...
	char **rownames)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_rownames called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	if (!rownames)
	{
		QSlog("mpq_ILLlib_rownames called with NULL rownames");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	int *intflags)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getintflags called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	char **colnames)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_colnames called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	if (!colnames)
	{
		QSlog("mpq_ILLlib_colnames called with NULL colnames");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	int *colindex)
{
	...
	*colindex = -1;
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_colindex called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	int *rowindex)
{
	...
	*rowindex = -1;
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_rowindex called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	char *rstat)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_getbasis called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	if (lp->basisid == -1)
	{
		QSlog("mpq_ILLlib_getbasis called with modifed LP");
		rval = 1;
		ILL_CLEANUP;
	}
	...
	for (i = 0; i < qslp->nstruct; i++)
	{
		...
		case STAT_ZERO:
			...
			break;
		default:
			QSlog("unknown vstat in mpq_ILLlib_getbasis: %d", lp->vstat[j]);
			rval = 1;
			ILL_CLEANUP;
		}
	}
	...
	for (i = 0; i < nrows; i++)
	{
		...
		if (qslp->rangeval && mpq_EGlpNumIsNeqqZero (qslp->rangeval[i]))
		{
			...
			case STAT_UPPER:
				...
				break;
			default:
				QSlog("unknown vstat in mpq_ILLlib_getbasis 2");
				rval = 1;
				ILL_CLEANUP;
			}
		}
		else
		{
			...
			case STAT_LOWER:
				...
				break;
			default:
				QSlog("unknown vstat in mpq_ILLlib_getbasis 3: %d, %d",
										i, lp->vstat[j]);
				rval = 1;
				ILL_CLEANUP;
			}
	...
	const char *fname)
{
	...
	in = mpq_ILLline_reader_new ((mpq_qsread_line_fct) EGioGets, file_in);
	rval = mpq_ILLmps_state_init (&state, in, fname);
	CHECKRVALG (rval, CLEANUP);
	...
	while (mpq_ILLmps_next_line (&state) == 0)
	{
		...
		else
		{
			...
			sec = ILLutil_index (mpq_ILLmps_section_name, state.key);
	...
	const char *fname)
{
	...
	if (!lp)
	{
		QSlog("mpq_ILLlib_writebasis called without an LP");
		rval = 1;
		ILL_CLEANUP;
	}
	if (!B && lp->basisid == -1)
	{
		QSlog("mpq_ILLlib_writebasis called with unsolved LP");
		rval = 1;
		ILL_CLEANUP;
	}
...
*/
	...
	if (basic != nrows)
	{
		QSlog("error in mpq_ILLlib_getrownorms");
		rval = 1;
		ILL_CLEANUP;
	}
...
int mpq_ILLlib_recompute_rownorms (
	mpq_lpinfo * lp,
	mpq_price_info * pinf)
{
	int rval = 0;
...
#define PRINT_TOL mpq_PFEAS_TOLER

int mpq_ILLlib_print_x (
	EGioFile_t * fd,
	mpq_lpinfo * lp,
	mpq_ILLlp_cache * C,
	mpq_t * x,
	int nonZerosOnly)
{
	...
	for (j = 0; j < qslp->nstruct; j++)
	{
		...
		if (!nonZerosOnly || mpq_EGlpNumIsNeqZero (dx[j], PRINT_TOL))
		{
