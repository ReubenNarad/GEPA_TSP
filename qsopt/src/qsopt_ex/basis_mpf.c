...
//#define DJZERO_TOLER mpf_PFEAS_TOLER
...
void mpf_ILLbasis_init_basisinfo (
	mpf_lpinfo * lp)
{
	lp->baz = 0;
	lp->nbaz = 0;
...
void mpf_ILLbasis_free_basisinfo (
	mpf_lpinfo * lp)
{
	...
	int strict)
{
	...
	if (lp->basisid == -1)
	{
		QSlog("mpf_ILLbasis_tableau_row: no basis");
	...
	if (rhs != NULL && trow != NULL)
	{
		...
		for (i = 0; i < lp->nrows; i++)
			if (mpf_EGlpNumIsGreatZero (tr[lp->baz[i]]))
		...
		if (mpf_EGlpNumIsLessZero (coef))
			mpf_EGlpNumSign (coef);
		if (mpf_EGlpNumIsLess (mpf_PIVZ_TOLER, coef))
			QSlog("tableau: bas computed = %.12f", mpf_EGlpNumToLf (sum));
		if (!strict)
	...
	int *vstat)
{
	...
	get_var_info (lp, &vd);
	if (!mpf_EGlpNumIsNeqqZero (vd.cmax))
		mpf_EGlpNumOne (cmax);
	else
	{
	...
	int *vstat)
{
	...
	for (k = 0; k < s_i; k++)
	{
		...
		for (j = 0; j < rcnt; j++)
		{
			...
			if (rcol[col] == 1)
				break;
			if (mpf_EGlpNumIsLessZero (dj[col]))
			{
 ...
 * mpf_CB_EPS = 0.001;
 * mpf_CB_PRI_RLIMIT = 0.25;
 * mpf_CB_INF_RATIO = 10.0; 
 * */
	int choice = 1;
	mpf_t rp, rd;
...
#if BASIS_STATS > 0
	QSlog("b1: nz %d pinf %.2f dinf %.2f", nz1, mpf_EGlpNumToLf (pinf1),
							mpf_EGlpNumToLf (dinf1));
	QSlog("b2: nz %d pinf %.2f dinf %.2f", nz2, mpf_EGlpNumToLf (pinf2),
	...
	int *singular)
{
	...
	*singular = 0;
	do
	{
		...
		rval = mpf_ILLfactor_create_factor_work (lp->f, lp->O->nrows);
		CHECKRVALG (rval, CLEANUP);
...
void mpf_ILLbasis_column_solve_update (
	mpf_lpinfo * lp,
	mpf_svector * rhs,
	mpf_svector * upd,
	mpf_svector * soln)
...
/* Bico - comment out for dist
       QSlog("Warning: numerically bad basis in mpf_ILLfactor_update");
*/
		*refactor = 1;
		rval = 0;
	}
