	...
	dbl_rawlpdata * lp)
{
	...
	if (skip)
	{
		dbl_ILLmps_set_end_of_line (state);	/* to avoid warning about extra fields */
	}
	else
	{
		if (strcmp (rhsname, " "))
	...
	dbl_qserror_collector * collector)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		...
		{
			str = dbl_EGlpNumGetStr(lp->rhs[i]);
			dbl_ILLprint_report (lp, " RHS    %s    %s\n", rownames[i], str);
			EGfree(str);
		}
	}
	...
	if (lp->rangeval)
	{
		...
		for (i = 0; i < lp->nrows; i++)
		{
			...
			{
				str = dbl_EGlpNumGetStr(lp->rangeval[i]);
				dbl_ILLprint_report (lp, " RANGE    %s    %s\n", rownames[i], str);
				EGfree(str);
			}
		}
	}
	...
	if (ri != lp->nstruct)
	{
		...
		for (; ri < lp->nstruct; ri++)
		{
			...
			{
				str = dbl_EGlpNumGetStr(lp->lower[i]);
				dbl_ILLprint_report (lp, " FX BOUND    %s    %s\n", colnames[ri], str);
				EGfree(str);
				continue;
			}
			...
			prtUpper = !dbl_ILLraw_default_upper (lp, i, ri);
			if (prtLower)
			{
				if (dbl_EGlpNumIsEqqual (lp->lower[i], dbl_ILL_MINDOUBLE))
				{
				...
				{
					str = dbl_EGlpNumGetStr(lp->lower[i]);
					dbl_ILLprint_report (lp, " LO BOUND    %s    %s\n", colnames[ri], str);
					EGfree(str);
				}
			}
			if (prtUpper)
			{
				...
				{
					str = dbl_EGlpNumGetStr(lp->upper[i]);
					dbl_ILLprint_report (lp, " UP BOUND    %s    %s\n", colnames[ri], str);
					EGfree(str);
				}
			}
		}
	...
	char *objname)
{
	...
	{
		str = dbl_EGlpNumGetStr(lp->obj[i]);
		dbl_ILLprint_report (lp, "  %s    %s    %s\n", colname, objname, str);
		EGfree(str);
	}
	for (k = A->matbeg[i]; k < A->matbeg[i] + A->matcnt[i]; k++)
	{
		row = A->matind[k];
		str = dbl_EGlpNumGetStr(A->matval[k]);
